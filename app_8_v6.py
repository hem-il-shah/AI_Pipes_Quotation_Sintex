...
# ══════════════════════════════════════════════════════════════════════════════
# CHANGE 3 — Azure Key Vault: fetch Doc Intelligence credentials
# ══════════════════════════════════════════════════════════════════════════════
_KV_URL = "https://kv-ai-pipes-quotation.vault.azure.net/"
...
def _fetch_keyvault_secret(secret_name: str, fallback_env: str) -> str:
    ...
    value = ""
    for credential_cls in [DefaultAzureCredential, ManagedIdentityCredential]:
        try:
            credential  = credential_cls()
            kv_client   = SecretClient(vault_url=_KV_URL, credential=credential)
            retrieved   = kv_client.get_secret(secret_name)
            value       = retrieved.value or ""
            if value:
                break
        except Exception:
            continue
...
def _get_azure_doc_intel_endpoint() -> str:
    return _fetch_keyvault_secret(
        "sc-doc-intel-endpoint",
        "AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT",
    )

def _get_azure_doc_intel_key() -> str:
    return _fetch_keyvault_secret(
        "sc-doc-intel-key",
...
# ══════════════════════════════════════════════════════════════════════════════
# CHANGE 2 — Azure Blob Storage configuration
# ══════════════════════════════════════════════════════════════════════════════
_AZURE_STORAGE_ACCOUNT   = "sintexpipesquotation01"
_AZURE_STORAGE_BASE_URL  = f"https://{_AZURE_STORAGE_ACCOUNT}.blob.core.windows.net"
_BLOB_CONTAINER_IMAGES   = "ocr-images"
_BLOB_CONTAINER_QUOTATIONS = "quotation-pdfs"
_BLOB_CONTAINER_DETECTIONS = "ocr-detections"

_AZURE_STORAGE_CONN_STR  = os.environ.get("AZURE_STORAGE_CONNECTION_STRING", "")


def _get_blob_service_client() -> BlobServiceClient:
    if _AZURE_STORAGE_CONN_STR:
        return BlobServiceClient.from_connection_string(_AZURE_STORAGE_CONN_STR)
    credential = DefaultAzureCredential()
    return BlobServiceClient(account_url=_AZURE_STORAGE_BASE_URL, credential=credential)


def _upload_to_blob(container: str, blob_name: str, data: bytes,
    ...
    cc.upload_blob(
        ...
        content_settings=__import__(
            "azure.storage.blob",
            fromlist=["ContentSettings"]
        ).ContentSettings(content_type=content_type),
    )
    return f"{_AZURE_STORAGE_BASE_URL}/{container}/{blob_name}"


def upload_image_to_blob(raw_bytes: bytes, rotation: int = 0) -> "tuple[str, str]":
    ts        = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    blob_name = f"order_{ts}.jpg"
    ...
    url = _upload_to_blob(_BLOB_CONTAINER_IMAGES, blob_name, data, "image/jpeg")
    return url, blob_name


def upload_quotation_pdf_to_blob(pdf_bytes: bytes, quotation_id: str) -> str:
    blob_name = f"{quotation_id}.pdf"
    return _upload_to_blob(
        _BLOB_CONTAINER_QUOTATIONS, blob_name, pdf_bytes, "application/pdf"
    )


def upload_detection_json_to_blob(detections: list, quotation_id: str) -> str:
    blob_name = f"detection_{quotation_id}.json"
    data      = json.dumps(detections, indent=2, ensure_ascii=False).encode("utf-8")
    return _upload_to_blob(
        _BLOB_CONTAINER_DETECTIONS, blob_name, data, "application/json"
    )
...
# ══════════════════════════════════════════════════════════════════════════════
SQL_SERVER   = 'sintex-quote-sql-server-dev.database.windows.net'
SQL_DATABASE = 'sintex-quote-dev-db'
SQL_DRIVER   = '{ODBC Driver 17 for SQL Server}'
SQL_USER     = 'swa-pipes-admin-login'
SQL_PASSWORD = 'SintexApps@123'
...
def db_insert_session_log(
    ...
    session_id = _build_session_id(ocr_id, quotation_id)
    sql = """
    INSERT INTO dbo.session_logs
        (session_id, ocr_id, quotation_id, ip_address,
         session_start_dt, session_end_dt)
...
/* Hide st.download_button default styling */
...
/* ── OCR Group Row separator (every 11 items) ── */
.ocr-group-separator td {
    ...
    border-bottom: 2px solid #C0211F !important;
}
...
MRP_PATH  = os.path.join(_BASE_DIR, "MERGED.xlsx")
...
# ══════════════════════════════════════════════════════════════════════════════
# CHANGE: load_mrp_data_for_state — strictly filters by state from Step-1 input.
# Returns only rows matching the selected state name from the "State Name" column.
# If no state is selected or no matching rows exist, returns an empty dict so
# that SKU codes not present in the state's catalogue are correctly marked as
# unpriced.
# ══════════════════════════════════════════════════════════════════════════════
@st.cache_data(show_spinner=False)
def load_mrp_data_for_state(state_name: str) -> dict:
    """
    Load MRP & distributor landing data from MERGED.xlsx, strictly filtered
    to rows where 'State Name' matches the state_name argument (case-insensitive).

    Returns:
        { material_number: { mrp, distributor_landing, description } }

    If state_name is blank/empty, returns empty dict (no pricing available).
    If no rows match the state, returns empty dict.
    """
    if not state_name or not state_name.strip():
        return {}

    try:
        df = pd.read_excel(MRP_PATH, engine="openpyxl")
    except Exception as exc:
        st.warning(f"⚠️ Could not read MERGED.xlsx: {exc}")
        return {}

    # ── Locate the State Name column ──────────────────────────────────────────
    state_col = None
    for candidate in ["State Name", "StateName", "State", "state_name", "state"]:
        if candidate in df.columns:
            state_col = candidate
            break

    # Without a state column we cannot filter — return empty
    if state_col is None:
        st.warning("⚠️ 'State Name' column not found in MERGED.xlsx. Cannot apply state filter.")
        return {}

    # ── Apply strict state filter ─────────────────────────────────────────────
    mask = df[state_col].astype(str).str.strip().str.lower() == state_name.strip().lower()
    state_df = df[mask]

    if state_df.empty:
        # No rows for this state — pricing will show as 0 / unpriced
        return {}

    # ── Locate MRP column ─────────────────────────────────────────────────────
    for col_candidate in ["MRP(ZPR1-933)", "MRP", "MRP(ZPR1)", "MRP (ZPR1-933)"]:
        if col_candidate in state_df.columns:
            mrp_col = col_candidate
            break
    else:
        mrp_candidates = [c for c in state_df.columns if "MRP" in str(c).upper()]
        mrp_col = mrp_candidates[0] if mrp_candidates else None

    # ── Locate Distributor Landing column ────────────────────────────────────
    for col_candidate in ["Distributor Landing", "Dist Landing", "Distributor_Landing",
                          "DistributorLanding", "Distributor  Landing"]:
        if col_candidate in state_df.columns:
            dist_col = col_candidate
            break
    else:
        dist_candidates = [c for c in state_df.columns if "DIST" in str(c).upper() or "LAND" in str(c).upper()]
        dist_col = dist_candidates[0] if dist_candidates else None

    # ── Locate Material Number column ─────────────────────────────────────────
    for col_candidate in ["Material Number", "Material", "Material No", "MaterialNumber",
                          "Material No.", "Mat No"]:
        if col_candidate in state_df.columns:
            mat_col = col_candidate
            break
    else:
        mat_candidates = [c for c in state_df.columns if "MAT" in str(c).upper()]
        mat_col = mat_candidates[0] if mat_candidates else state_df.columns[0]

    # ── Locate Description column ─────────────────────────────────────────────
    for col_candidate in ["Material Description", "Description", "Mat Desc",
                          "Material Desc", "Product Description"]:
        if col_candidate in state_df.columns:
            desc_col = col_candidate
            break
    else:
        desc_col = None
    ...
    if mrp_col:
        ...
        state_df["_m"] = pd.to_numeric(
            state_df[mrp_col].astype(str).str.replace(",", "").str.strip(), errors="coerce"
        ).fillna(0)
    else:
        state_df = state_df.copy()
        state_df["_m"] = 0.0
    ...
    if dist_col:
        state_df["_d"] = pd.to_numeric(
            state_df[dist_col].astype(str).str.replace(",", "").str.strip(), errors="coerce"
        ).fillna(0)
    else:
        state_df["_d"] = 0.0
...
def load_state_names_from_csv() -> list:
    """Load unique state names from MERGED.xlsx. Falls back to ALL_INDIA_STATES."""
    try:
        df = pd.read_excel(MRP_PATH, engine="openpyxl")
        state_col = None
        for candidate in ["State Name", "StateName", "State", "state_name", "state"]:
        ...
        if state_col:
            ...
            states = [s for s in states if s and s not in ("nan", "None", "")]
            if states:
...
# ══════════════════════════════════════════════════════════════════════════════
# CHANGE 1 — Azure Document Intelligence prebuilt-layout OCR
# ══════════════════════════════════════════════════════════════════════════════

def run_document_intelligence_ocr(img_bytes: bytes,
                                   endpoint: str,
                                   key: str) -> list:
    """
    Use Azure Document Intelligence prebuilt-layout to extract table data.
    Returns a list of raw detection dicts:
      { description, sku, size, quantity, formatted }
    """
    ...
    with io.BytesIO(img_bytes) as img_stream:
        poller = di_client.begin_analyze_document(
            "prebuilt-layout", document=img_stream
        )
    ...
    for table in result.tables:
        ...
        for cell in table.cells:
            if cell.row_index in (0, 1) and cell.column_index >= 2:
                raw_hdr = cell.content.replace("\n", " ").strip()
                if raw_hdr:
                    header_map[cell.column_index] = raw_hdr

        if not header_map:
            continue

        norm_header_map: dict[int, str] = {}
        for col_idx, raw_hdr in header_map.items():
            cleaned = raw_hdr.upper().replace(" ", "").rstrip(".")
            canonical = SIZE_ALIASES.get(cleaned) or SIZE_ALIASES.get(
                re.sub(r"[^0-9]", "", cleaned) + "MM",
                raw_hdr,
            )
            norm_header_map[col_idx] = canonical

        row_desc: dict[int, str] = {}
        row_sku:  dict[int, str] = {}
        ...
        for cell in table.cells:
            ...
            if ci < 2:
                continue
            if ci not in norm_header_map:
                continue

            content = cell.content.strip()
            if not content or not content.isdigit() or int(content) <= 0:
                continue

            description = row_desc.get(ri, "").strip()
            sku_prefix  = row_sku.get(ri, "").strip()
            size_label  = norm_header_map[ci]
            quantity    = content
...
def match_detections_to_sku(
    raw_detections: list,
    pidx: dict,
    master: dict,
) -> "tuple[dict, list, list]":
    """
    Convert raw Document Intelligence detections into the quantities / match_log
    / line_items structures expected by the rest of the application.

    Returns:
        quantities  – { full_sku: total_qty }
        match_log   – list of log-entry dicts
        line_items  – list of { sku, qty } ordered dicts
    """
    quantities: dict  = {}
    match_log:  list  = []
    line_items: list  = []
    ...
    if not raw_detections:
        return quantities, match_log, line_items

    for det in raw_detections:
        description = det.get("description", "")
        sku_prefix  = det.get("sku", "")
        size_label  = det.get("size", "")
        qty_str     = det.get("quantity", "0")
        ...
        full_sku = match_sku(sku_prefix, size_label, pidx, master)

        if full_sku:
            quantities[full_sku] = quantities.get(full_sku, 0) + qty
            line_items.append({"sku": full_sku, "qty": qty})
        ...
        match_log.append({
            ...
            "size":     size_label if size_label else "—",
            "qty":      qty,
            "full_sku": full_sku if full_sku else "—",
            "status":   "matched" if full_sku else "unmatched",
        })

    return quantities, match_log, line_items


# ── build_live_log (unchanged logic) ──────────────────────────────────────────
def build_live_log(
    match_log: list,
    quantities: dict,
    line_items: list,
    ...
    live: list = []
    skus_in_match_log: set = set()

    for entry in match_log:
        row = dict(entry)

        if row["status"] == "matched" and row["full_sku"] != "—":
            skus_in_match_log.add(row["full_sku"])
            row["qty"] = quantities.get(row["full_sku"], row["qty"])
...
def build_pdf(quantities: dict, mrp_data: dict, bill_to: dict,
    ...
    hdr_ctr_sub = ps("hcs",fontName="Times-Roman",  fontSize=8,  textColor=WHITE, alignment=TA_CENTER, leading=13)
    hdr_rt      = ps("hr", fontName="Times-Roman",  fontSize=8,  textColor=WHITE, alignment=TA_RIGHT,  leading=13)
    ...
    heads = ["S.No", "Product", "SKU Code", "OD", "Inch", "MRP (₹)", "Qty",
             "Total MRP (₹)", "Dist. Landing (₹)", "Taxable (₹)"]
...
# ── MRP data is loaded using the currently selected state from session state ──
mrp_data         = load_mrp_data_for_state(st.session_state.selected_state)
available_states = load_state_names_from_csv()


# ══════════════════════════════════════════════════════════════════════════════
# CHANGE: build_product_size_map — now accepts mrp_data to restrict available
# products/sizes strictly to those present in the state-filtered pricing data.
# Products whose SKU codes do NOT appear in mrp_data are excluded from the
# Edit/Add dropdown so users can only add items that have valid state pricing.
# ══════════════════════════════════════════════════════════════════════════════
@st.cache_data(show_spinner=False)
def build_product_size_map(sku_master_frozen: str, mrp_data_frozen: str) -> dict:
    """
    Build { product_name: { od_size: sku_code } } mapping.

    Only includes SKU codes that:
      1. Exist in the full sku_master catalogue.
      2. Have a corresponding entry in mrp_data (state-filtered pricing).

    This ensures the Edit/Add dropdown only shows products/sizes available
    for the currently selected state.
    """
    master   = json.loads(sku_master_frozen)
    mrp      = json.loads(mrp_data_frozen)
    pmap: dict = {}

    for sku, info in master.items():
        # Only include SKUs present in state-filtered pricing data
        if sku not in mrp:
            continue
...
def validate_party(d: dict, prefix: str) -> list:
    ...
    if not name:
        errors.append(f"{prefix} Party Name is mandatory and cannot be empty.")
    if not mobile:
        errors.append(f"{prefix} Mobile Number is mandatory and cannot be empty.")
    if not address:
        errors.append(f"{prefix} Address is mandatory and cannot be empty.")
...
def has_mandatory_fields(d: dict) -> bool:
    return bool(
        d.get("name", "").strip() and
        d.get("mobile", "").strip() and
        d.get("address", "").strip()
...
def render_step1():
    ...
    with st.expander("🔧 Azure OCR / Key Vault Settings", expanded=False):
        kv_endpoint = _get_azure_doc_intel_endpoint()
        kv_key      = _get_azure_doc_intel_key()

        if kv_endpoint and kv_key:
            st.markdown(
                '<div class="success-box">'
                '✅ <b>Azure Key Vault credentials loaded successfully.</b><br/>'
                f'Endpoint: <code>{kv_endpoint[:55]}…</code>'
                '</div>',
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                '<div class="warn-box">'
                '⚠️ Key Vault credentials not yet loaded. '
                'Falling back to environment variables or manual override below.'
                '</div>',
                unsafe_allow_html=True,
            )
            manual_ep  = st.text_input(
                "Azure Endpoint (manual override)",
                value=st.session_state.get("azure_endpoint", ""),
                placeholder="https://YOUR-RESOURCE.cognitiveservices.azure.com",
            )
            manual_key = st.text_input(
                "Azure API Key (manual override)",
                value=st.session_state.get("azure_key", ""),
                type="password",
                placeholder="••••••••••••••••••••••••••••••••",
            )
            if manual_ep:
                st.session_state["azure_endpoint"] = manual_ep
            if manual_key:
                st.session_state["azure_key"] = manual_key
    ...
    if raw_bytes_pending is not None:
        ...
        if st.button("✅  Submit & Process Image", key="btn_submit_image"):
            for k in ["quantities", "match_log", "ocr_grid", "ocr_meta",
                      "ocr_extracted", "line_items", "raw_detections"]:
            ...
            st.session_state.image_blob_url           = ""
            st.session_state.quotation_blob_url       = ""
            st.session_state.detection_blob_url       = ""
            st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
...
def render_step2():
    ...
    if (st.session_state.image_submitted
        ...
        try:
            ...
            st.warning(f"⚠️ Azure Blob / DB write warning (Step 2 init): {exc}")
    ...
    <div class="step-card"><div class="step-card-header">
      ...
      <div>
        <div class="step-title">Step 2 — OCR · Document Intelligence · SKU Matching</div>
        <div class="step-subtitle">
          Uses Azure Document Intelligence prebuilt-layout — extracts tables, matches SKUs
          {f'&nbsp;·&nbsp; State: <b>{st.session_state.selected_state}</b>' if st.session_state.selected_state else ''}
        </div>
      </div>
    </div><div class="step-body">""", unsafe_allow_html=True)

    # ── State pricing banner ───────────────────────────────────────────────────
    selected_state = st.session_state.get("selected_state", "")
    if not selected_state:
        st.markdown(
            '<div class="warn-box">⚠️ <b>No state selected.</b> Please go back to Step 1 and select a state. '
            'Pricing cannot be applied without a selected state.</div>',
            unsafe_allow_html=True,
        )
    else:
        n_priced_skus = len(mrp_data)
        if n_priced_skus > 0:
            st.markdown(
                f'<div class="success-box">✅ <b>State pricing active: {selected_state}</b> — '
                f'{n_priced_skus:,} SKU pricing records loaded from MERGED.xlsx. '
                f'SKU matching and colour-coding are based on this state\'s pricing data.</div>',
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                f'<div class="warn-box">⚠️ <b>No pricing data found for "{selected_state}"</b> in MERGED.xlsx. '
                f'All matched SKUs will appear as "Matched — No Price". '
                f'Verify the State Name column in MERGED.xlsx matches exactly.</div>',
                unsafe_allow_html=True,
            )

    # ── Resolve credentials ───────────────────────────────────────────────────
    active_endpoint = _get_azure_doc_intel_endpoint() or st.session_state.get("azure_endpoint", "")
    active_key      = _get_azure_doc_intel_key()      or st.session_state.get("azure_key", "")

    if not done:
        c1, c2 = st.columns(2)
        with c1:
            run = st.button(
                "Run OCR & Match", key="run_ocr",
                disabled=not (active_key and active_endpoint and st.session_state.image_bytes),
            )
        with c2:
        ...
        if not st.session_state.image_bytes:
            st.markdown(
                '<div class="error-box">❌ No image captured — go back to Step 1.</div>',
                unsafe_allow_html=True,
            )
        if not active_endpoint:
            st.markdown(
                '<div class="warn-box">⚠️ Azure Endpoint not available — check Key Vault or manual override in Step 1.</div>',
                unsafe_allow_html=True,
            )
        if not active_key:
            st.markdown(
                '<div class="warn-box">⚠️ Azure API Key not available — check Key Vault or manual override in Step 1.</div>',
                unsafe_allow_html=True,
            )
        ...
        if run:
            ph = st.empty()
            try:
                ph.info("⏳ 1/3 — Enhancing image…")
                enhanced_bytes = enhance_image_for_ocr(
                    st.session_state.image_bytes,
                    st.session_state.get("image_rotation", 0),
                )

                ph.info("⏳ 2/3 — Running Azure Document Intelligence (prebuilt-layout)…")
                raw_detections = run_document_intelligence_ocr(
                    enhanced_bytes, active_endpoint, active_key
                )

                ph.info(f"🔍 3/3 — Matching {len(raw_detections)} detection(s) to SKU catalogue…")
                quantities, log, line_items = match_detections_to_sku(
                    raw_detections, prefix_index, sku_master
                )
                ...
                if nm == 0:
                    ph.warning(
                        f"⚠️ Document Intelligence found {len(raw_detections)} quantity cell(s) "
                        f"but 0 SKUs matched. Check the Edit / Add tab below."
                    )
                else:
                    ph.success(
                        f"✅ Done — {nm} detections matched, {nu} unmatched. Review below."
                    )
                time.sleep(0.8)
                st.rerun()
    ...
    if done:
        ...
        if nm:
            st.markdown(
                f'<div class="success-box">✅ {nm} SKU-size detections matched → {nm} line items in document</div>',
                unsafe_allow_html=True,
            )
        if nu:
            st.markdown(
                f'<div class="warn-box">⚠️ {nu} pair(s) unmatched — review in Edit / Add tab</div>',
                unsafe_allow_html=True,
            )
        if not nm and not nu:
            st.markdown(
                '<div class="warn-box">No OCR data — use Edit tab to add SKUs manually.</div>',
                unsafe_allow_html=True,
            )

        tab_edit, tab_log = st.tabs(["✏️ Edit / Add", "📋 OCR Detection Log"])
        ...
        with tab_edit:
            ...
            if items_existing:
                ...
                st.markdown(
                    ...
                    'Prices shown are from the <b>' + (selected_state or "selected") + ' state pricing</b>. '
                ...
                table_height = max(44 + (n_rows * 48) + 44 + 72 + 40, 320)
...
tbody td{{padding:8px 10px;color:#1A1A1A;vertical-align:middle;border-right:1px solid #EFEFEF;}}
...
tfoot td{{padding:9px 10px;font-weight:700;font-size:12.5px;color:#1A1A1A;border-right:1px solid #DEDEDE;}}
...
.actions-bar{{display:flex;align-items:center;justify-content:space-between;gap:12px;
    margin-top:14px;padding:12px 4px 4px;border-top:1.5px solid #EFEFEF;
    flex-wrap:wrap;min-height:56px;}}
.hint-text{{font-size:11.5px;color:#92400E;background:#FFFBEB;border:1.5px solid #FDE68A;
    border-radius:8px;padding:8px 14px;font-weight:500;flex-shrink:0;}}
.apply-btn{{background:linear-gradient(135deg,#1E7E4A,#155d38);color:white;border:none;
    border-radius:9px;padding:12px 28px;font-family:'Inter',sans-serif;font-size:13.5px;
    font-weight:700;cursor:pointer;box-shadow:0 4px 14px rgba(30,126,74,0.3);
...
.applied-msg{{display:none;color:#065F46;font-size:12px;font-weight:600;
    background:#ECFDF5;border:1.5px solid #A7F3D0;border-radius:6px;padding:8px 14px;flex-shrink:0;}}
</style>
</head>
<body>
<div class="tbl-wrap">
<table id="edit-tbl">
  <thead>
    <tr>
      ...
      <th class="R">Total MRP (Rs.)</th>
      <th class="R">Dist. Landing / Unit (₹)</th>
      <th class="R">Taxable Value (Rs.)</th>
    </tr>
  </thead>
...
function onQtyChange(inp, idx){{
    ...
    if(qty !== orig){{ inp.classList.add('changed'); }}
...
function applyChanges(){{
    ...
    var result = [];
    inputs.forEach(function(inp){{
        result.push({{ sku: inp.getAttribute('data-sku'), qty: parseInt(inp.value) || 0 }});
    }});
...
buildTable();
            ...
            # ══════════════════════════════════════════════════════════════════
            # CHANGE: Add Item by Product & Size
            # Now uses build_product_size_map with mrp_data to restrict the
            # product/size dropdowns to only those available in the selected
            # state's pricing data.
            # ══════════════════════════════════════════════════════════════════
            st.markdown('<div class="fsl">Add Item by Product &amp; Size</div>', unsafe_allow_html=True)

            # Show a contextual note about state-filtering
            if selected_state and mrp_data:
                st.markdown(
                    f'<div class="info-box" style="margin-bottom:10px;padding:8px 14px;">'
                    f'🗺️ Showing products available for <b>{selected_state}</b> state pricing only. '
                    f'Products not present in this state\'s catalogue are excluded.'
                    f'</div>',
                    unsafe_allow_html=True,
                )
            elif selected_state and not mrp_data:
                st.markdown(
                    f'<div class="warn-box" style="margin-bottom:10px;padding:8px 14px;">'
                    f'⚠️ No pricing data available for <b>{selected_state}</b>. '
                    f'The product list below is empty. Please verify MERGED.xlsx.'
                    f'</div>',
                    unsafe_allow_html=True,
                )

            _master_json = json.dumps(sku_master)
            _mrp_json    = json.dumps(mrp_data)
            pmap         = build_product_size_map(_master_json, _mrp_json)
            ...
            with col_prod:
                if product_names:
                    ...
                    st.selectbox(
                        "Product Name",
                        options=["— no products available for this state —"],
                        key="add_product_select_empty",
                        label_visibility="collapsed",
                        disabled=True,
                    )
                    sel_product = None

            if sel_product and sel_product not in ("— select product —", None):
                avail_sizes = sorted(
                    pmap[sel_product].keys(),
            ...
            with col_qty:
                add_qty = st.number_input(
                    ...
                    key="add_qty_input", label_visibility="collapsed",
                )
            ...
            with col_add:
                ...
                if st.button("ADD", key="add_by_product", disabled=(resolved_sku is None)):
                    ...
                    st.success(
                        f"Added: {info_added.get('product', resolved_sku)} "
                        f"({sel_size}) × {add_qty} → SKU: {resolved_sku}"
                    )
                    st.rerun()

            if resolved_sku:
                mi_resolved = mrp_data.get(resolved_sku, {})
                mrp_disp    = f"₹ {mi_resolved.get('mrp', 0):,.2f}" if mi_resolved else "—"
                dist_disp   = f"₹ {mi_resolved.get('distributor_landing', 0):,.2f}" if mi_resolved else "—"
                st.markdown(
                    f'<div class="info-box" style="margin-top:6px;padding:8px 12px;">'
                    f'🔗 Resolved SKU: <code><b>{resolved_sku}</b></code>'
                    f'&nbsp;&nbsp;|&nbsp;&nbsp;MRP: <b>{mrp_disp}</b>'
                    f'&nbsp;&nbsp;|&nbsp;&nbsp;Dist. Landing: <b>{dist_disp}</b>'
                    f'</div>',
                    unsafe_allow_html=True,
                )
            elif (sel_product and sel_product not in ("— select product —", None)
                    and sel_size and sel_size != "— size —" and avail_sizes):
                st.markdown(
        ...
        # ══════════════════════════════════════════════════════════════════════
        # CHANGE: OCR DETECTION LOG TAB
        # Displays detections in groups of 11 with separator rows.
        # Colour-coding is based strictly on state-filtered mrp_data:
        #   🟢 Green  — matched AND has valid MRP price in selected state
        #   🟠 Orange — matched BUT MRP = 0 (not in state's pricing catalogue)
        #   🔴 Red    — unmatched (SKU prefix could not be resolved)
        # ══════════════════════════════════════════════════════════════════════
        with tab_log:
            ...
            # ── State pricing context banner ───────────────────────────────────
            if selected_state:
                st.markdown(
                    f'<div class="info-box" style="margin-bottom:12px;">'
                    f'🗺️ <b>Pricing context: {selected_state}</b> — '
                    f'Colour-coding (🟢/🟠/🔴) reflects whether each matched SKU '
                    f'has a valid MRP entry in MERGED.xlsx for this state.'
                    f'</div>',
                    unsafe_allow_html=True,
                )
            ...
            if live_log:
                ...
                # ── Summary banner ────────────────────────────────────────────
                total_rows_with_qty = sum(1 for e in live_log if e["qty"] > 0)
                st.markdown(
                    f'<div class="info-box" style="margin-bottom:10px;">'
                    f'📊 <b>Total log entries: {len(live_log)}</b> &nbsp;·&nbsp; '
                    f'Rows with quantity: <b>{total_rows_with_qty}</b> &nbsp;·&nbsp; '
                    f'Raw DI detections: <b>{len(raw_detections)}</b>'
                    f'</div>',
                    unsafe_allow_html=True,
                )

                # ── CHANGE: Main colour-coded table with groups of 11 ─────────
                # Every 11 data rows, insert a visual separator row showing the
                # group number, so large OCR results are easy to navigate.
                # Group counting is 1-based and only counts actual data rows
                # (separators are not counted).
                # ─────────────────────────────────────────────────────────────
                GROUP_SIZE = 11

                h = (
                    '<div class="ocr-wrap"><table class="ocr-tbl">'
                    '<thead><tr>'
                    '<th style="text-align:center;width:36px">#</th>'
                    '<th class="L">Product / Description</th>'
                    '<th class="L">OCR SKU Prefix</th>'
                    '<th>Size</th>'
                    '<th>Qty</th>'
                    '<th class="L">Resolved SKU</th>'
                    '<th>MRP / Unit (₹)</th>'
                    '<th>Dist. Landing / Unit (₹)</th>'
                    '<th>Total MRP (₹)</th>'
                    '<th>Status</th>'
                    '</tr></thead><tbody>'
                )

                n_green = n_orange = n_red = 0
                total_cols = 10  # number of <td> columns in the table

                for idx, entry in enumerate(live_log, 1):

                    # ── Insert group separator before every GROUP_SIZE-th item ─
                    # idx is 1-based; insert separator BEFORE items 1, 12, 23, …
                    if (idx - 1) % GROUP_SIZE == 0:
                        group_num    = ((idx - 1) // GROUP_SIZE) + 1
                        group_start  = idx
                        group_end    = min(idx + GROUP_SIZE - 1, len(live_log))
                        h += (
                            f'<tr class="ocr-group-separator">'
                            f'<td colspan="{total_cols}">'
                            f'GROUP {group_num} &nbsp;·&nbsp; '
                            f'Items {group_start}–{group_end}'
                            f'</td>'
                            f'</tr>'
                        )
                    ...
                    if status == "unmatched":
                        ...
                        # SKU matched in catalogue but not priced for this state
                        row_cls    = "log-row-orange"
                        status_lbl = "🟠 No State Price"
                        mrp_disp   = "₹ 0.00"
                        dist_disp  = "₹ 0.00"
                        total_disp = "₹ 0.00"
                        n_orange += 1
                        ...
                        status_lbl = "🟢 Matched &amp; Priced"
                        mrp_disp   = f"₹ {mrp_val:,.2f}"
                        dist_disp  = f"₹ {dist_val:,.2f}"
                        total_disp = f"₹ {total_mrp:,.2f}"
                        n_green += 1

                    sku_display = (
                        f'<code style="font-size:10px;background:#f0f0f0;padding:1px 4px;'
                        f'border-radius:3px;font-family:monospace;">{sku}</code>'
                        if sku != "—"
                        else '<span style="color:#ccc">—</span>'
                    )
                    qty_display = (
                        f'<b style="color:#1E7E4A;">{qty}</b>'
                        if qty > 0
                    ...
                    h += (
                        ...
                        f'<td class="L">{entry["product"] or """<span style="color:#aaa">—</span>"""}</td>'
                        f'<td class="M">{entry["prefix"] or """<span style="color:#aaa">—</span>"""}</td>'
                ...
                <div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:10px;padding:10px 0;">
                  ...
                  <div style="background:#FFF7ED;border:1.5px solid #FED7AA;border-radius:8px;
                      ...
                      🟠 {n_orange} Matched — No State Price
                  </div>
                ...
                # ── Raw Document Intelligence output (collapsible) ────────────
                if raw_detections:
                    with st.expander(
                        f"📄 Raw Document Intelligence Output ({len(raw_detections)} detections)",
                        expanded=False,
                    ):
                        st.markdown(
                            '<div class="info-box" style="margin-bottom:10px;">'
                            'ℹ️ These are the raw cell values extracted by Azure Document Intelligence '
                            '<b>prebuilt-layout</b> before SKU matching. '
                            'Format: <code>Description: SKU_Prefix SIZE: Quantity</code>'
                            '</div>',
                            unsafe_allow_html=True,
                        )
                        raw_h = (
                            '<div class="raw-wrap"><table class="raw-tbl">'
                            '<thead><tr>'
                            '<th>#</th>'
                            '<th>Description</th>'
                            '<th>SKU Prefix</th>'
                        ...
                        for ri, det in enumerate(raw_detections, 1):
                            # Group separator every 11 raw detections
                            if (ri - 1) % GROUP_SIZE == 0:
                                grp_n = ((ri - 1) // GROUP_SIZE) + 1
                                raw_h += (
                                    f'<tr style="background:#1A1A1A;">'
                                    f'<td colspan="6" style="color:#AAAAAA;font-size:10px;'
                                    f'font-weight:700;text-align:center;padding:5px 8px;'
                                    f'letter-spacing:0.8px;text-transform:uppercase;'
                                    f'border-bottom:2px solid #C0211F;">'
                                    f'GROUP {grp_n} &nbsp;·&nbsp; '
                                    f'Detections {ri}–{min(ri + GROUP_SIZE - 1, len(raw_detections))}'
                                    f'</td>'
                                    f'</tr>'
                                )
                            raw_h += (
                                f'<tr>'
                                f'<td style="color:#999;text-align:center">{ri}</td>'
                                f'<td>{det.get("description","")}</td>'
                                f'<td class="sku">{det.get("sku","")}</td>'
                                f'<td>{det.get("size","")}</td>'
                                f'<td class="qty">{det.get("quantity","")}</td>'
                                f'<td style="font-size:10.5px;color:#555;">'
                                f'{det.get("formatted","")}'
                                f'</td>'
                                f'</tr>'
                            )
                        raw_h += "</tbody></table></div>"
                        st.markdown(raw_h, unsafe_allow_html=True)
        ...
        disc  = gmrp - gdist
        n_li  = len(st.session_state.line_items) if st.session_state.line_items else len(iord)
        st.markdown(f"""
        <div class="totals-box">
          <div class="total-row">
            <span class="total-lbl">Line items</span>
            <span class="total-val">{n_li}</span>
          </div>
          <div class="total-row">
          ...
          <div class="total-row grand">
            <span class="total-lbl">Net Distributor Landing</span>
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 3 — Dealer / Distributor Details (unchanged)
# ══════════════════════════════════════════════════════════════════════════════
def render_step3():
    ...
    st.markdown(
        ...
        '⚠️ Fields marked with <b style="color:#C0211F;">*</b> are mandatory. '
        'Party Name, Mobile Number, and Address must be filled for both Bill To and Ship To '
        'before the quotation can be generated.'
        '</div>',
        unsafe_allow_html=True,
    )

    state_options = ALL_INDIA_STATES
    ...
    st.markdown('**Distributor Name** <span class="mandatory-label">*</span>', unsafe_allow_html=True)
    bill_name = st.text_input("Distributor Name", key="bill_na",
                              placeholder="Full name / business name (required)",
                              label_visibility="collapsed")
    st.markdown('**Address** <span class="mandatory-label">*</span>', unsafe_allow_html=True)
    bill_address = st.text_area("Address", key="bill_addr",
                                placeholder="Street, area, city, PIN code (required)",
    ...
    with bc3:
        st.markdown('**Phone** (+91)', unsafe_allow_html=True)
        bill_ph = st.text_input("+91 Phone", key="bill_ph",
                                label_visibility="collapsed", placeholder="10-digit number")
    with bc4:
        st.markdown('**Mobile** (+91) <span class="mandatory-label">*</span>', unsafe_allow_html=True)
        bill_mb = st.text_input("+91 Mobile", key="bill_mb",
                                label_visibility="collapsed", placeholder="10-digit number (required)")
    bc5, bc6 = st.columns(2)
    with bc5:
        st.markdown('**State**', unsafe_allow_html=True)
        try:
            bill_state_idx = state_options.index(st.session_state.get("bill_state_dropdown", ""))
        except ValueError:
            bill_state_idx = 0
        bill_st = st.selectbox("State (Bill To)", options=state_options,
                               index=bill_state_idx, key="bill_state_select",
                               label_visibility="collapsed")
        st.session_state["bill_state_dropdown"] = bill_st
    with bc6:
    ...
    st.markdown('**Customer Name** <span class="mandatory-label">*</span>', unsafe_allow_html=True)
    ship_name = st.text_input("Customer Name", key="ship_na",
                              placeholder="Full name / business name (required)",
                              label_visibility="collapsed")
    st.markdown('**Address** <span class="mandatory-label">*</span>', unsafe_allow_html=True)
    ship_address = st.text_area("Address", key="ship_addr",
                                placeholder="Street, area, city, PIN code (required)",
                                height=80, label_visibility="collapsed")
    ...
    with sc3:
        st.markdown('**Phone** (+91)', unsafe_allow_html=True)
        ship_ph = st.text_input("+91 Phone", key="ship_ph",
                                label_visibility="collapsed", placeholder="10-digit number")
    with sc4:
        st.markdown('**Mobile** (+91) <span class="mandatory-label">*</span>', unsafe_allow_html=True)
        ship_mb = st.text_input("+91 Mobile", key="ship_mb",
                                label_visibility="collapsed", placeholder="10-digit number (required)")
    sc5, sc6 = st.columns(2)
    with sc5:
        st.markdown('**State**', unsafe_allow_html=True)
        try:
            ship_state_idx = state_options.index(st.session_state.get("ship_state_dropdown", ""))
        except ValueError:
            ship_state_idx = 0
        ship_st = st.selectbox("State (Ship To)", options=state_options,
                               index=ship_state_idx, key="ship_state_select",
                               label_visibility="collapsed")
        st.session_state["ship_state_dropdown"] = ship_st
    with sc6:
    ...
    with sm1:
        margin_pct_field = st.number_input(
            ...
            step=0.5, key="extra_margin_percent",
        )
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    bill_mandatory_ok = has_mandatory_fields(bill_to)
    ship_mandatory_ok = has_mandatory_fields(ship_to)
    mandatory_complete = bill_mandatory_ok and ship_mandatory_ok
    ...
    def do_confirm(b: dict, s: dict):
        ...
        with st.spinner("Generating PDF…"):
            try:
                pdf_bytes = build_pdf(
                    st.session_state.quantities, mrp_data, b, s,
                    sku_master, qid, st.session_state.get("line_items", []),
                )
                st.session_state.pdf_bytes = pdf_bytes

                q_blob_url = upload_quotation_pdf_to_blob(pdf_bytes, qid)
                st.session_state.quotation_blob_url = q_blob_url

                try:
                    det_blob_url = upload_detection_json_to_blob(
                        st.session_state.get("raw_detections", []), qid
                    )
                    st.session_state.detection_blob_url = det_blob_url
                except Exception as det_exc:
                    st.warning(f"⚠️ Detection JSON upload warning: {det_exc}")
                    det_blob_url = ""
                ...
                if not st.session_state.quotation_db_written:
                    db_insert_quotation_log(
                        ...
                        ocr_id              = st.session_state.db_ocr_id,
                        pdf_link            = q_blob_url,
                        detection_pdf_link  = det_blob_url,
                        bill_to             = b,
                        ship_to             = s,
                ...
                try:
                    append_log_entry(qid, q_blob_url, b, s, b.get("name",""), margin_pct_field)
                except Exception:
                    pass
                ...
                st.error(f"PDF / Azure Blob / DB error: {exc}")

    if is_party_complete(bill_to) and is_party_complete(ship_to) and not all_errors:
        st.markdown(
            '<div class="success-box">✅ All fields filled — generating PDF automatically…</div>',
            unsafe_allow_html=True,
        )
    ...
    format_errors_only = [
        ...
        if "mandatory" not in e.lower() and "cannot be empty" not in e.lower()
    ]
    has_format_errors = bool(format_errors_only)

    if mandatory_complete and not has_format_errors:
        if st.button("Generate Quotation", key="confirm"):
            do_confirm(bill_to, ship_to)
    elif not mandatory_complete:
        st.markdown(
            '<div style="opacity:0.5;cursor:not-allowed;margin-top:8px;">'
            '<div style="background:linear-gradient(135deg,#C0211F,#8B1514);color:white;'
            'border:none;border-radius:10px;padding:13px 20px;font-size:14.5px;font-weight:600;'
            'text-align:center;box-shadow:0 4px 14px rgba(192,33,31,.28);">'
            '⛔ Fill all mandatory fields (*) to Generate Quotation'
            '</div></div>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            ...
            'Fix the validation errors above, then generate the quotation.</div>',
            unsafe_allow_html=True,
        )
        st.markdown(
            '<div style="opacity:0.5;cursor:not-allowed;margin-top:8px;">'
            ...
            'border:none;border-radius:10px;padding:13px 20px;font-size:14.5px;font-weight:600;'
            'text-align:center;box-shadow:0 4px 14px rgba(192,33,31,.28);">'
            '⛔ Fix Format Errors to Generate Quotation'
            '</div></div>',
            unsafe_allow_html=True,
        )
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 4 — Preview & Download (unchanged)
# ══════════════════════════════════════════════════════════════════════════════
def render_step4():
    ...
    if quotation_id:
        ...
        session_end_badge = (
            '<span class="session-end-badge stamped">✅ Session closed — download / share confirmed</span>'
            if session_ended
            else '<span class="session-end-badge pending">⏳ Session end stamped on first download or WhatsApp share</span>'
        )

        q_blob = st.session_state.get("quotation_blob_url", "")
        blob_badge = (
            f'<a href="{q_blob}" target="_blank" rel="noopener noreferrer" '
            f'style="font-size:10px;color:#1d4ed8;text-decoration:underline;margin-left:8px;">'
            f'☁️ Azure Blob</a>'
            if q_blob else ""
        )
    ...
    if st.session_state.pdf_bytes:
        ...
        <div class="totals-box" style="margin-bottom:20px;">
          <div class="total-row">
            <span class="total-lbl">Line Items</span><span class="total-val">{n_lines}</span>
          </div>
          <div class="total-row">
            <span class="total-lbl">Gross MRP</span><span class="total-val">₹ {gmrp:,.2f}</span>
          </div>
          <div class="total-row">
            <span class="total-lbl">Distributor Discount</span>
            <span class="total-val neg">− ₹ {disc:,.2f}</span>
          ...
          <div class="total-row grand">
            <span class="total-lbl">Net Taxable Value</span><span class="total-val">₹ {gdist:,.2f}</span>
          </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col_wa:
            ...
            if st.button("📲  Open WhatsApp", key="wa_btn",
                         use_container_width=True, on_click=_wa_click):
                pass
            st.markdown('</div>', unsafe_allow_html=True)
...
window.parent.open({json.dumps(_open_url)}, '_blank', 'noopener,noreferrer');
        ...
        with col_new:
            ...
            if st.button("🆕  New Quotation", key="new_quote_btn",
                         use_container_width=True):
                _reset_session()
            st.markdown('</div>', unsafe_allow_html=True)
...
def _reset_session():
    ...
    for k in ["quantities", "bill_to", "ship_to", "ocr_meta"]:
        st.session_state[k] = {}
    for k in ["match_log", "ocr_grid", "ocr_extracted", "line_items", "raw_detections"]:
        st.session_state[k] = []
    st.session_state["ocr_done"]               = False
    st.session_state["ocr_reviewed"]           = False
    st.session_state["party_confirmed"]        = False
    ...
    st.session_state["bill_state_dropdown"]    = ""
    st.session_state["ship_state_dropdown"]    = ""
    st.rerun()
