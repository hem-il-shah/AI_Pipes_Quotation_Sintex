...
(function() {
  function fixHeader() {
    ...
    const scrollContainer = document.querySelector('section[data-testid="stAppViewContainer"] > div:first-child')
                         || document.querySelector('.main > div')
                         || document.querySelector('.block-container')?.parentElement;
    if (!scrollContainer) { setTimeout(fixHeader, 100); return; }
    // Clone into a fixed portal
    ...
    // Hide original, add top padding so content isn't hidden behind sticky bar
    header.style.visibility = 'hidden';
    header.style.height = header.offsetHeight + 'px';
    const ph = portal.offsetHeight;
    document.body.style.paddingTop = ph + 'px';
    // Keep content from being hidden under fixed bar by offsetting main container
...
def load_mrp_data_for_state(state_name: str) -> dict:
    """
    Load MRP data from the CSV. The CSV is expected to have a 'State Name' column.
    Rows are filtered to match the selected state. Falls back to all rows if no
    'State Name' column exists (backward-compatible with the original single-state file).
    """
    ...
    # Filter by state if the column exists
...
def load_state_names_from_csv() -> list[str]:
    """
    Read unique state names from the 'State Name' column of the MRP CSV.
    Falls back to the hardcoded ALL_INDIA_STATES list if the column is absent.
    """
    try:
        df = pd.read_csv(MRP_PATH, encoding="latin-1")
...
# ═══════════════════════════════════════════════════════════════════════════════
# PDF GENERATION — formal B&W / grey palette, Times New Roman fonts
# ═══════════════════════════════════════════════════════════════════════════════
...
# ─── Build product→SKU lookup for the Edit/Add manual entry ───────────────────
# Maps: product_name → {size_label → sku_code}
# e.g. {"Ball Valve CPVC": {"15MM": "CPF11BV...", "20MM": "CPF11BV..."}}
@st.cache_data(show_spinner=False)
def build_product_size_map(sku_master_frozen: str) -> dict:
    """
    Build a nested dict: product_name -> od_size -> sku_code
    Accepts a JSON-serialised sku_master to satisfy cache hashing.
    """
...
# ═══════════════════════════════════════════════════════════════════════════════
# STEP 1 — CAPTURE  (State selector is INSIDE this step, above the camera/upload)
# ═══════════════════════════════════════════════════════════════════════════════
...
def render_step1():
    ...
    # ── STATE SELECTOR (inside Step 1 body) ───────────────────────────────────
    try:
    ...
    st.markdown('</div>', unsafe_allow_html=True)  # close state-inner-card
    ...
    if not st.session_state.image_submitted:
            ...
            with tab_cam:
                ...
                if cam:
                    ...
                    st.session_state.raw_image_bytes = cam.getvalue()
            ...
            with tab_up:
                uf = st.file_uploader("Upload order form image", type=["jpg","jpeg","png"],
                                    label_visibility="collapsed",
    ...
    if st.session_state.image_bytes is not None:
        ...
        if not st.session_state.image_submitted:
            # ── IMAGE EDITOR ────────────────────────────────────────────────
            st.markdown('<div class="success-box">✅ Image loaded — edit if needed, then click <b>Save &amp; Continue</b></div>',
                        unsafe_allow_html=True)
...
<style>
  *{{box-sizing:border-box;margin:0;padding:0;font-family:'Inter',-apple-system,sans-serif;}}
  ...
  .toolbar{{display:flex;flex-wrap:wrap;gap:6px;padding:10px;background:#2a2a2a;
  ...
  .img-wrap{{width:100%;height:420px;background:#111;overflow:hidden;}}
  .img-wrap img{{max-width:100%;display:block;}}
  .save-bar{{padding:10px;background:#2a2a2a;border-top:1px solid #444;display:flex;gap:8px;}}
  .save-bar button{{flex:1;padding:11px;border:none;border-radius:8px;
    font-size:14px;font-weight:700;cursor:pointer;}}
  .btn-save{{background:linear-gradient(135deg,#C0211F,#8B1514);color:white;}}
  .btn-reset{{background:#444;color:white;}}
  .btn-save:hover{{background:#8B1514;}}
  #status{{font-size:11px;color:#aaa;padding:4px 10px;background:#1a1a1a;text-align:center;}}
</style>
</head>
<body>
<div class="toolbar">
  <button onclick="cropper.rotate(-90)" title="Rotate Left">↺ -90°</button>
  <button onclick="cropper.rotate(90)"  title="Rotate Right">↻ +90°</button>
  <button onclick="cropper.rotate(-45)" class="secondary">↺ -45°</button>
  <button onclick="cropper.rotate(45)"  class="secondary">↻ +45°</button>
  <div class="sep"></div>
  <button onclick="cropper.scaleX(-cropper.getData().scaleX||1)" class="secondary">⇄ Flip H</button>
  <button onclick="cropper.scaleY(-cropper.getData().scaleY||1)" class="secondary">⇅ Flip V</button>
  <div class="sep"></div>
  <button onclick="cropper.zoom(0.1)"  class="secondary">🔍+</button>
  <button onclick="cropper.zoom(-0.1)" class="secondary">🔍−</button>
  <div class="sep"></div>
  <button onclick="setCropMode()" class="secondary" id="cropBtn">✂️ Crop Mode</button>
  <button onclick="setMoveMode()" class="secondary" id="moveBtn">✋ Move Mode</button>
</div>
<div class="img-wrap">
  <img id="image" src="data:image/jpeg;base64,{b64_raw}" style="max-height:420px;"/>
</div>
<div id="status">Drag to crop · Scroll to zoom · Use toolbar to rotate/flip</div>
<div class="save-bar">
  <button class="btn-reset" onclick="resetImage()">↩ Reset</button>
...
<script>
  ...
  function saveImage(){{
    ...
    const canvas = cropper.getCroppedCanvas({{maxWidth:3000, maxHeight:3000, fillColor:'#fff'}});
...
"""
            ...
            # ── Listener for postMessage from editor ─────────────────────────
            saved_b64_key = f"saved_img_b64_{upload_key_suffix}"
            recv_html = f"""
<script>
window.addEventListener('message', function(e){{
  if(e.data && e.data.type === 'sintex_img_save'){{
    const ta = document.createElement('textarea');
    ta.id   = 'sintex_img_payload';
    ta.style.display = 'none';
    ta.value = e.data.data;
    document.body.appendChild(ta);
    // Also set into a hidden Streamlit text_input via DOM
...
"""
            ...
            # Hidden text input to receive base64 from JS postMessage
            img_payload = st.text_input("__img_payload__", key=f"img_payload_{upload_key_suffix}",
            ...
            # Manual fallback submit (in case postMessage doesn't fire)
...
def render_step2():
    ...
    if done:
        ...
        with tab_edit:
            ...
            # Available sizes for chosen product
            ...
            with col_add:
                ...
                if st.button("➕", key="add_by_product", disabled=(resolved_sku is None)):
                    ...
                    # Also update line_items to preserve order
            ...
            # Show resolved SKU for transparency
            if resolved_sku:
                st.markdown(
            ...
            # ── Direct SKU lookup (power user fallback) ──────────────────────
...
def render_step3():
    ...
    def party_form(pfx, title, emoji):
        ...
        # State and PAN side by side in same row
        c5, c6 = st.columns(2)
        with c5:
...
# Changes:
#   • "Edit Parties" button removed
#   • 3 buttons in one row: Download Quotation | Send on WhatsApp | New Quotation
# ═══════════════════════════════════════════════════════════════════════════════
...
def render_step4():
    ...
    if st.session_state.pdf_bytes:
        ...
        # ── 3-button row: Download | Send on WhatsApp | New Quotation ─────────
        ...
        # PDF not yet generated — only show New Quotation
