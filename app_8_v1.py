...
# ── CHANGE 1: Updated from MRP_State_chhattisghar.csv to MERGED.xlsx ──────────
...
# ══════════════════════════════════════════════════════════════════════════════
# CHANGE 1: Updated MRP data loader — reads from MERGED.xlsx instead of CSV
# Supports both "State Name" column (xlsx) and falls back gracefully.
# ══════════════════════════════════════════════════════════════════════════════
@st.cache_data(show_spinner=False)
def load_mrp_data_for_state(state_name: str) -> dict:
    ...
    # Normalise numeric columns — handle comma-formatted numbers
    for col_candidate in ["MRP(ZPR1-933)", "MRP", "MRP(ZPR1)", "MRP (ZPR1-933)"]:
        if col_candidate in df.columns:
            mrp_col = col_candidate
            break
    else:
        # Try to find any column whose name contains 'MRP'
    ...
    # Filter by state if column exists and state is provided
...
# ══════════════════════════════════════════════════════════════════════════════
# CHANGE 2: build_live_log — shows ALL OCR-detected entries without skipping.
#
# Root cause of the bug: The original function was iterating match_log and
# skipping entries whose qty had dropped to 0 in quantities dict. It also
# de-duplicated by ocr_sku_set which caused some entries to be hidden.
#
# Fix: Emit every single entry from match_log (which represents every raw
# OCR detection) regardless of current quantity, and additionally append
# any manually-added items that were not part of the original OCR log.
# ══════════════════════════════════════════════════════════════════════════════
def build_live_log(
    match_log: list,
    quantities: dict,
    ...
    """
    Return a list of every detected (or manually-added) entry to display in
    the OCR Detection Log tab.

    Rules:
    - Every entry in match_log is emitted exactly once (preserving original
      OCR row order). Unmatched entries are kept as-is. Matched entries
      show the CURRENT quantity from the quantities dict (so edits are
      reflected) — but the row is never hidden even if qty dropped to 0.
    - Manually-added SKUs that do not appear anywhere in match_log are
      appended at the end.
    """
    live: list = []
    # Track which SKUs have already been represented via match_log rows
    seen_in_match_log: set = set()
    ...
    for entry in match_log:
        ...
        if status == "unmatched":
            # Always show unmatched rows — they tell the user what was detected
            # but could not be resolved to a product SKU.
            live.append(dict(entry))
            continue

        # Matched row — reflect the current (possibly edited) quantity.
        current_qty = quantities.get(sku, entry["qty"])
        updated = dict(entry)
        updated["qty"] = current_qty
        live.append(updated)
        seen_in_match_log.add(sku)

    # Append manually-added items that did not originate from OCR
    seen_manual: set = set()
    for item in line_items:
        sku = item["sku"]
        if sku in seen_in_match_log:
            continue
        ...
        # Still include even if qty == 0 so the user can see it was added
...
# ══════════════════════════════════════════════════════════════════════════════
# CHANGE 3: Updated validate_party — now enforces mandatory fields:
#   Bill To:  name (Party Name), mobile, address
#   Ship To:  name (Customer Name), mobile, address
# ══════════════════════════════════════════════════════════════════════════════
def validate_party(d: dict, prefix: str) -> list:
    ...
    # ── Format validation (only when value is provided) ──────────────────────
...
def is_party_complete(d: dict) -> bool:
    """
    Auto-generate check: requires the full set of fields.
    (Used for the auto-confirm shortcut — all fields must be filled.)
    """
    return bool(
...
def has_mandatory_fields(d: dict) -> bool:
    """
    Mandatory-only check used to gate the Generate Quotation button:
    Name, Mobile, and Address must be present.
    """
    return bool(
        d.get("name", "").strip() and
        d.get("mobile", "").strip() and
...
# ── Insert session log on first load ──────────────────────────────────────────
...
def render_step2():
    ...
    if done:
        ...
        with tab_edit:
            ...
            # ── Section A: OCR-Detected Items — inline editable HTML table ───
        ...
        with tab1:
            ...
            if live_log:
                ...
                st.markdown(
                    ...
                    f'📊 Total detections shown: <b>{len(live_log)}</b> '
                    f'(all OCR-detected rows, including unmatched)'
                    f'</div>',
                    unsafe_allow_html=True,
                )
...
# CHANGE 3: Mobile, Party Name, and Address are now mandatory for both
#           Bill To and Ship To. The form cannot proceed unless these three
#           fields are filled for both parties.
# ══════════════════════════════════════════════════════════════════════════════
def render_step3():
    ...
    bill_address_label_col, _ = st.columns([3, 1])
    st.markdown(
    ...
    # ── Validation ────────────────────────────────────────────────────────────
    # validate_party now checks mandatory fields (name, mobile, address) first,
    # then format checks for phone/mobile/party_no.
    all_errors = validate_party(bill_to, "Bill To") + validate_party(ship_to, "Ship To")

    # Check mandatory-only for gate logic (subset of all_errors for UX)
    ...
    # ── Show which mandatory fields are still missing (inline real-time hints) ─
    missing_hints = []
    if not bill_to.get("name", "").strip():
        missing_hints.append("Bill To: Distributor Name")
    if not bill_to.get("mobile", "").strip():
        missing_hints.append("Bill To: Mobile Number")
    if not bill_to.get("address", "").strip():
        missing_hints.append("Bill To: Address")
    if not ship_to.get("name", "").strip():
        missing_hints.append("Ship To: Customer Name")
    if not ship_to.get("mobile", "").strip():
        missing_hints.append("Ship To: Mobile Number")
    if not ship_to.get("address", "").strip():
        missing_hints.append("Ship To: Address")

    if missing_hints:
        hints_html = "".join(
            f'<li style="margin:2px 0;">{h}</li>' for h in missing_hints
        )
        st.markdown(
            f'<div class="warn-box" style="margin-top:10px;">'
            f'<b>⚠️ Mandatory fields still required:</b>'
            f'<ul style="margin:6px 0 0 16px;padding:0;">{hints_html}</ul>'
            f'</div>',
            unsafe_allow_html=True,
        )
    ...
    # ── Auto-generate if ALL fields complete (original behaviour preserved) ──
    ...
    # ── Generate button — gated on mandatory fields + no format errors ────────
    # "mandatory_complete" = name, mobile, address filled for both parties.
    # "all_errors" = includes mandatory-missing AND format errors.
    # Button is enabled only when mandatory fields are present and no format
    # errors exist (format errors cannot be bypassed via the main button).
    format_errors_only = [
    ...
    if mandatory_complete and not has_format_errors:
        ...
        # Show a disabled-style button with explanatory text
        st.markdown(
        ...
        # Format errors present — show errors and a disabled button
        st.markdown(
