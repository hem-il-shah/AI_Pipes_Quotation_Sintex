...
/* ── Detection Log colour rows ── */
.det-row-green td {
    background: #ECFDF5 !important;
    color: #065F46 !important;
}
.det-row-green td.mono-cell {
    color: #065F46 !important;
}
.det-row-orange td {
...
# ══════════════════════════════════════════════════════════════════════════════
# CHANGED FUNCTION: build_quantities
# Now returns 4 values: quantities, match_log, line_items, raw_ocr_detections
# raw_ocr_detections captures EVERY detected product+size+qty pair without
# any filtering. This is what the Detection Log tab displays.
# ══════════════════════════════════════════════════════════════════════════════
def build_quantities(extracted_rows: list, pidx: dict, master: dict):
    """
    Process OCR-extracted rows to build quantities, match_log, line_items,
    and raw_ocr_detections.

    raw_ocr_detections: every (product, sku_prefix, size, qty, raw_cell_text)
    pair found by OCR, with no filtering. Used for the Detection Log tab.

    Returns: (quantities, match_log, line_items, raw_ocr_detections)
    """
    line_items       = []
    quantities       = {}
    match_log        = []
    raw_ocr_detections = []   # ← NEW: unfiltered full OCR output

    for row in extracted_rows:
        # ── Always record the row in raw_ocr_detections, even if sizes is empty ──
        # A row with no detected quantity still shows up so the user knows OCR
        # saw the product name / SKU prefix but found no handwritten number.
        if not row["sizes"]:
            # Row was detected by OCR (product name / SKU found) but no qty
            # was successfully parsed from any size column.
            raw_ocr_detections.append({
                "product":    row["product"],
                "sku_prefix": row["sku_prefix"],
                "size":       "",
                "qty":        None,          # None = detected but no qty parsed
                "full_sku":   "—",
                "status":     "no_qty",
            })
            continue
        ...
        for sz, qty in row["sizes"].items():
            # ── Build the raw detection record for this (product, size, qty) ──
            full_sku = match_sku(row["sku_prefix"], sz, pidx, master)
            ...
            # ── Build the filtered outputs (same logic as before) ──
            if full_sku:
    ...
    return quantities, match_log, line_items, raw_ocr_detections
...
def build_live_log(
    ...
    """
    Return ONLY entries where a handwritten quantity was actually detected (qty > 0).
    Rows with no quantity written on the form are excluded entirely.
    Every entry comes from a real OCR-detected number in the image.
    """
    live: list = []

    skus_in_match_log: set = set()
    for entry in match_log:
        if entry["status"] == "matched" and entry["full_sku"] != "—":
            skus_in_match_log.add(entry["full_sku"])
...
for k, v in [
    ...
    # ── NEW: raw unfiltered OCR detections for the Detection Log tab ──
    ("raw_ocr_detections", []),
    ...
    # Step-3 state dropdown selections (persisted so they survive reruns)
    ("bill_state_dropdown", ""),
    ("ship_state_dropdown", ""),
]:
    _ss(k, v)
...
(function(){
    ...
    if raw_bytes_pending is not None:
        ...
        if st.button("✅  Submit & Process Image", key="btn_submit_image"):
            for k in ["quantities", "match_log", "ocr_grid", "ocr_meta", "ocr_extracted",
                      "line_items", "raw_ocr_detections"]:
    ...
    if not done:
        ...
        if run:
            ...
            try:
                ...
                # ── CHANGED: build_quantities now returns 4 values ──
                qtys, log, line_items, raw_detections = build_quantities(
                    res["rows"], prefix_index, sku_master
                )
                ...
                # ── NEW: store raw detections ──
                st.session_state.raw_ocr_detections = raw_detections
        ...
        if skip:
            ...
            st.session_state.raw_ocr_detections = []
            st.session_state.ocr_done           = True
            if st.session_state.db_ocr_id:
                db_update_ocr_log(ocr_id=st.session_state.db_ocr_id,
                                  quantities_detected=0, matched_count=0,
...
buildTable();
        ...
        # ══════════════════════════════════════════════════════════════════════
        # OCR DETECTION LOG TAB — FULLY REWRITTEN
        # Renders from raw_ocr_detections (complete unfiltered JSON).
        # Colour logic:
        #   🟢 GREEN  → status == "matched"  AND  mrp_data has a price > 0
        #   🟠 ORANGE → status == "matched"  AND  mrp_data has price == 0 (no pricing loaded)
        #   🔴 RED    → status == "unmatched" (OCR found qty but SKU couldn't be resolved)
        # ══════════════════════════════════════════════════════════════════════
        with tab1:
            raw_detections = st.session_state.get("raw_ocr_detections", [])

            # Filter: only show rows where a real quantity was actually read
            # (status "no_qty" rows had product/SKU detected but no number written)
            det_with_qty = [d for d in raw_detections if d.get("qty") is not None]

            if det_with_qty:
                # ── Legend ────────────────────────────────────────────────────
                st.markdown("""
                <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:14px;align-items:center;">
                  <span style="font-size:11.5px;font-weight:700;color:#6B6B6B;
                      text-transform:uppercase;letter-spacing:.6px;">Legend:</span>
                  <span style="background:#ECFDF5;color:#065F46;border:1.5px solid #A7F3D0;
                      border-radius:6px;padding:4px 12px;font-size:11.5px;font-weight:600;">
                      🟢 Matched &amp; Priced
                  </span>
                ...
                # ── Summary banner ────────────────────────────────────────────
                n_green  = sum(
                    1 for d in det_with_qty
                    if d["status"] == "matched" and mrp_data.get(d["full_sku"], {}).get("mrp", 0) > 0
                )
                n_orange = sum(
                    1 for d in det_with_qty
                    if d["status"] == "matched" and mrp_data.get(d["full_sku"], {}).get("mrp", 0) == 0
                )
                n_red    = sum(1 for d in det_with_qty if d["status"] == "unmatched")

                st.markdown(
                    f'<div class="info-box" style="margin-bottom:14px;">'
                    f'📊 <b>Total OCR Detections (with quantity): {len(det_with_qty)}</b> — '
                    f'Every product row where a handwritten quantity was read from the image is listed below, '
                    f'without any filtering.'
                    f'</div>',
                    unsafe_allow_html=True,
                )

                # ── JSON export block ─────────────────────────────────────────
                # Build the structured JSON for display / download
                json_export = []
                for d in det_with_qty:
                    sku      = d["full_sku"]
                    info     = sku_master.get(sku, {}) if sku != "—" else {}
                    mi       = mrp_data.get(sku, {})   if sku != "—" else {}
                    json_export.append({
                        "product":          d["product"],
                        "sku_prefix_ocr":   d["sku_prefix"],
                        "size_detected":    d["size"],
                        "qty_detected":     d["qty"],
                        "resolved_sku":     sku,
                        "product_master":   info.get("product", ""),
                        "od_size":          info.get("od_size", ""),
                        "inch_size":        info.get("inch_size", ""),
                        "mrp":              mi.get("mrp", 0),
                        "distributor_landing": mi.get("distributor_landing", 0),
                        "status":           d["status"],
                    })

                json_str = json.dumps(json_export, indent=2, ensure_ascii=False)

                with st.expander("📄 View Raw Detection JSON", expanded=False):
                    st.code(json_str, language="json")
                    st.download_button(
                        label="⬇️ Download JSON",
                        data=json_str.encode("utf-8"),
                        file_name=f"ocr_detections_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
                ...
                # ── Detection table ───────────────────────────────────────────
                st.markdown(
                    '<div style="margin:14px 0 8px;font-size:12px;font-weight:700;'
                    'color:#1A1A1A;text-transform:uppercase;letter-spacing:.6px;">'
                    'Complete Detection Table</div>',
                    unsafe_allow_html=True,
                )

                h = """<div class="ocr-wrap">
                <table class="ocr-tbl" style="font-size:11.5px;">
                <thead><tr>
                  <th style="text-align:center;width:32px;">#</th>
                  <th class="L" style="min-width:130px;">Product (OCR)</th>
                  <th class="L" style="min-width:90px;">SKU Prefix (OCR)</th>
                  <th style="min-width:50px;">Size</th>
                  <th style="min-width:60px;">Qty (Detected)</th>
                  <th class="L" style="min-width:100px;">Resolved SKU</th>
                  <th class="L" style="min-width:130px;">Product (Master)</th>
                  <th style="min-width:50px;">OD</th>
                  <th style="min-width:50px;">Inch</th>
                  <th style="min-width:80px;">MRP (₹)</th>
                  <th style="min-width:100px;">Dist. Landing (₹)</th>
                  <th style="min-width:80px;">Status</th>
                </tr></thead><tbody>"""

                for idx, d in enumerate(det_with_qty, 1):
                    sku      = d["full_sku"]
                    status   = d["status"]
                    info     = sku_master.get(sku, {}) if sku != "—" else {}
                    mi       = mrp_data.get(sku, {})   if sku != "—" else {}
                    ...
                    if status == "unmatched":
                        ...
                        mrp_disp   = '<span style="color:#ccc;">—</span>'
                        dist_disp  = '<span style="color:#ccc;">—</span>'
                        sku_disp   = '<span style="color:#ccc;font-style:italic;">Not found</span>'
                        prod_mast  = '<span style="color:#ccc;font-style:italic;">—</span>'
                        od_disp    = '<span style="color:#ccc;">—</span>'
                        inch_disp  = '<span style="color:#ccc;">—</span>'
                    elif mrp_val == 0:
                        row_cls    = "det-row-orange"
                        status_lbl = "🟠 No Price"
                        mrp_disp   = '<span style="font-family:\'JetBrains Mono\',monospace;">₹ 0.00</span>'
                        dist_disp  = '<span style="font-family:\'JetBrains Mono\',monospace;">₹ 0.00</span>'
                        sku_disp   = f'<code style="font-size:10px;font-weight:700;">{sku}</code>'
                        prod_mast  = info.get("product", "—")
                        od_disp    = f'<b>{info.get("od_size","—")}</b>'
                        inch_disp  = info.get("inch_size", "—")
                    else:
                        row_cls    = "det-row-green"
                        status_lbl = "🟢 Matched"
                        mrp_disp   = f'<span style="font-family:\'JetBrains Mono\',monospace;font-weight:600;">₹ {mrp_val:,.2f}</span>'
                        dist_disp  = f'<span style="font-family:\'JetBrains Mono\',monospace;">₹ {dist_val:,.2f}</span>'
                        sku_disp   = f'<code style="font-size:10px;font-weight:700;">{sku}</code>'
                        prod_mast  = info.get("product", "—")
                        od_disp    = f'<b>{info.get("od_size","—")}</b>'
                        inch_disp  = info.get("inch_size", "—")

                    qty_display = f'<b style="font-size:14px;">{d["qty"]}</b>'

                    h += (
                        f'<tr class="{row_cls}">'
                        f'<td style="text-align:center;color:#999;font-size:10px;">{idx}</td>'
                        f'<td class="L" style="font-weight:500;font-size:11px;">{d["product"] or "—"}</td>'
                        f'<td class="M" style="font-size:10px;">{d["sku_prefix"] or "—"}</td>'
                        f'<td><b>{d["size"]}</b></td>'
                        f'<td>{qty_display}</td>'
                        f'<td class="L">{sku_disp}</td>'
                        f'<td class="L" style="font-size:11px;">{prod_mast}</td>'
                        f'<td>{od_disp}</td>'
                        f'<td>{inch_disp}</td>'
                        f'<td>{mrp_disp}</td>'
                        f'<td>{dist_disp}</td>'
                        f'<td><b style="white-space:nowrap;">{status_lbl}</b></td>'
                        f'</tr>'
                    )
                ...
                # ── Summary badges ────────────────────────────────────────────
                h += f"""
                <div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:14px;padding:10px 0;">
                  ...
                  <div style="background:#F0F0F0;border:1.5px solid #D0D0D0;border-radius:8px;
                      padding:8px 16px;font-size:12.5px;font-weight:700;color:#333;">
                      📊 {len(det_with_qty)} Total Detections
                  </div>
                </div>
                """
                ...
                st.markdown(
                    '<div class="warn-box">No handwritten quantities were detected in this image. '
...
def render_step3():
    ...
    st.markdown(
        '**Distributor Name** <span class="mandatory-label">*</span>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '**Address** <span class="mandatory-label">*</span>',
        unsafe_allow_html=True,
    )
    bill_address = st.text_area(
        "Address",
        key="bill_addr",
    ...
    with bc4:
        st.markdown(
            '**Mobile** (+91) <span class="mandatory-label">*</span>',
            unsafe_allow_html=True,
        )
        bill_mb = st.text_input(
            "+91 Mobile",
            key="bill_mb",
            label_visibility="collapsed",
    ...
    with bc5:
        ...
        try:
            ...
            bill_state_idx = 0
        bill_st = st.selectbox(
            "State (Bill To)",
            options=state_options,
            index=bill_state_idx,
            key="bill_state_select",
            label_visibility="collapsed",
        )
    ...
    st.markdown(
        '**Customer Name** <span class="mandatory-label">*</span>',
        unsafe_allow_html=True,
    )
    ...
    with sc5:
        ...
        try:
            ...
            ship_state_idx = 0
        ship_st = st.selectbox(
            "State (Ship To)",
            options=state_options,
            index=ship_state_idx,
            key="ship_state_select",
            label_visibility="collapsed",
        )
...
def _reset_session():
    ...
    for k in ["quantities", "bill_to", "ship_to", "ocr_meta"]:
        st.session_state[k] = {}
    for k in ["match_log", "ocr_grid", "ocr_extracted", "line_items", "raw_ocr_detections"]:
        st.session_state[k] = []
    st.session_state["ocr_done"]                 = False
    st.session_state["ocr_reviewed"]             = False
    st.session_state["party_confirmed"]          = False
