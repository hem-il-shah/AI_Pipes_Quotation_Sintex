...
/* Locked step card */
.step-card-locked{background:white;border-radius:var(--radius);box-shadow:var(--shadow);
  margin-bottom:20px;overflow:hidden;border:1.5px dashed var(--border);opacity:0.55;}
.step-card-locked .step-card-header{background:#999;padding:14px 18px;
  display:flex;align-items:center;gap:12px;}
...
for k, v in [
    ...
    ("ocr_reviewed", False),      # FIX 2: explicit gate for Step 3
    ("upload_key", 0),            # FIX 1: incrementing key forces widget reset on retake
]:
    _ss(k, v)
...
def validate_party(d: dict, prefix: str) -> list[str]:
    ...
    # if sc and not re.fullmatch(r'[A-Za-z0-9]+', sc):
    #     errors.append(f"{prefix} State Code must be alphanumeric only.")
    return errors
...
# FIX 2: Step 3 now requires ocr_reviewed (set only by the "Proceed" button),
#         not just ocr_done.
step_unlocked = [
    True,                                      # Step 1: always
    st.session_state.image_submitted,          # Step 2: after Submit clicked
    st.session_state.ocr_reviewed,             # Step 3: after "Proceed to Party Details" clicked
...
def render_step1():
    ...
    # FIX 1: Only show upload widgets when not yet submitted.
    # The upload_key counter is used to force Streamlit to re-mount the widgets
    # after Retake, which clears any cached file/camera value.
    ...
    if st.session_state.image_bytes is not None:
        ...
        if not st.session_state.image_submitted:
            ...
            with col_retake:
                ...
                # FIX 1: increment upload_key so camera/uploader widgets are
                # re-mounted with a new key, clearing any cached image value.
                if st.button("🔄 Retake", key="retake_img"):
                    ...
                    st.session_state.party_confirmed = False
                    st.session_state.pdf_bytes       = None
                    st.session_state.upload_key     += 1   # forces widget re-mount
                    for k in ["quantities","match_log","ocr_grid","ocr_meta","ocr_extracted","line_items"]:
                        st.session_state[k] = [] if isinstance(st.session_state[k], list) else {}
                    st.rerun()
            ...
            if st.session_state.ocr_done:
                ...
                st.markdown('<div class="success-box">✅ Image submitted — proceed to Step 2 for OCR</div>',
                            unsafe_allow_html=True)
            st.markdown('<div class="btn-secondary">', unsafe_allow_html=True)
            if st.button("🔄  Upload Different Image", key="change_img"):
                st.session_state.image_bytes     = None
                st.session_state.image_submitted = False
                st.session_state.ocr_done        = False
                st.session_state.ocr_reviewed    = False
...
def render_step2():
    if not step_unlocked[1]:
        st.markdown("""
        <div class="step-card-locked"><div class="step-card-header">
          <div class="step-number">2</div>
          <div><div class="step-title">Step 2 — OCR · Spatial Reconstruction · SKU Matching</div>
          <div class="step-subtitle">🔒 Upload an image and click Submit in Step 1 first</div></div>
        </div></div>""", unsafe_allow_html=True)
        return
    ...
    if done:
        ...
        # FIX 2: clicking this button sets ocr_reviewed=True, which is the
        # gate that unlocks Step 3 in step_unlocked[2].
...
def render_step3():
    if not step_unlocked[2]:
        ...
        <div class="step-card-locked"><div class="step-card-header">
          ...
          <div class="step-subtitle">🔒 Complete OCR review and click "Proceed to Party Details" first</div></div>
        </div></div>""", unsafe_allow_html=True)
        return
    ...
    def party_form(pfx, title, emoji):
        ...
        with c3:
            ...
            ph = st.text_input("+91 Phone", value=d.get("phone",""), key=f"{pfx}_ph",
                               label_visibility="collapsed",
                               placeholder="10-digit number")
        with c4:
            st.markdown("**Mobile** (+91)")
            mb = st.text_input("+91 Mobile", value=d.get("mobile",""), key=f"{pfx}_mb",
                               label_visibility="collapsed",
                               placeholder="10-digit number")

        c5, c6 = st.columns(2)

        with c5: st_ = st.text_input("State",     value=d.get("state",""),      key=f"{pfx}_st")
    ...
    btn_label = "Confirm & Generate PDF →  Step 4"
    if not all_errors:
...
def render_step4():
    if not step_unlocked[3]:
        ...
        <div class="step-card-locked"><div class="step-card-header">
          ...
          <div class="step-subtitle">🔒 Complete party details first</div></div>
        </div></div>""", unsafe_allow_html=True)
        return
