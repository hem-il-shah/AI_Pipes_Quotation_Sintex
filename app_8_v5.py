...
# Vault: https://kv-ai-pipes-quotation.vault.azure.net/
# Secrets: sc-doc-intel-key  /  sc-doc-intel-endpoint
# Falls back to environment variables if Key Vault is unreachable.
# ══════════════════════════════════════════════════════════════════════════════
_KV_URL = "https://kv-ai-pipes-quotation.vault.azure.net/"

def _fetch_keyvault_secret(secret_name: str, fallback_env: str) -> str:
    """
    Attempt to read *secret_name* from Azure Key Vault.
    Falls back to the environment variable *fallback_env* on any error.
    Result is cached in st.session_state so the round-trip happens only once.
    """
    ...
    value = ""
    # Try Managed Identity first (works on Azure App Service / ACI), then env var.
...
# Storage Account : sintexpipesquotation01
# Containers      : ocr-images | quotation-pdfs | ocr-detections
# ══════════════════════════════════════════════════════════════════════════════
...
# Connection string / SAS — pulled from env; set this in App Service / .env
_AZURE_STORAGE_CONN_STR  = os.environ.get("AZURE_STORAGE_CONNECTION_STRING", "")


def _get_blob_service_client() -> BlobServiceClient:
    """Return a BlobServiceClient using connection string or Managed Identity."""
...
def _upload_to_blob(container: str, blob_name: str, data: bytes,
    ...
    Returns the public blob URL (without SAS — adjust if containers are private).
    Raises on error so callers can decide how to handle.
    """
...
def upload_image_to_blob(raw_bytes: bytes, rotation: int = 0) -> "tuple[str, str]":
    """
    Pre-process (EXIF fix + rotate) then upload to ocr-images.
...
def upload_quotation_pdf_to_blob(pdf_bytes: bytes, quotation_id: str) -> str:
    """Upload quotation PDF to quotation-pdfs container. Returns blob URL."""
...
def upload_detection_json_to_blob(detections: list, quotation_id: str) -> str:
    """Upload OCR detection JSON to ocr-detections container. Returns blob URL."""
...
def get_db_connection():
    """Return a live pyodbc connection using SQL Server Authentication (Azure)."""
    conn_str = (
        f'DRIVER={SQL_DRIVER};'
        f'SERVER={SQL_SERVER};'
...
# ══════════════════════════════════════════════════════════════════════════════
# LOCAL FOLDER SETUP (kept for Excel log only; file storage moved to Azure Blob)
# ══════════════════════════════════════════════════════════════════════════════
...
# Azure Doc Intelligence credentials resolved at runtime (Change 3)
# (accessed via _get_azure_doc_intel_endpoint() / _get_azure_doc_intel_key())
...
# ── File helpers (Azure Blob — Change 2) ──────────────────────────────────────
# save_image_to_disk / save_quotation_pdf_to_disk / save_detection_pdf_to_disk
# are replaced by upload_image_to_blob / upload_quotation_pdf_to_blob /
# upload_detection_json_to_blob defined at the top of this file.
...
def load_mrp_data_for_state(state_name: str) -> dict:
    """Load MRP & distributor landing data from MERGED.xlsx for a given state."""
    try:
    ...
    for col_candidate in ["MRP(ZPR1-933)", "MRP", "MRP(ZPR1)", "MRP (ZPR1-933)"]:
        ...
        mrp_candidates = [c for c in df.columns if "MRP" in str(c).upper()]
    ...
    for col_candidate in ["Distributor Landing", "Dist Landing", "Distributor_Landing",
        ...
        dist_candidates = [c for c in df.columns if "DIST" in str(c).upper() or "LAND" in str(c).upper()]
    ...
    for col_candidate in ["Material Number", "Material", "Material No", "MaterialNumber",
        ...
        mat_candidates = [c for c in df.columns if "MAT" in str(c).upper()]
        mat_col = mat_candidates[0] if mat_candidates else df.columns[0]
    ...
    if mrp_col:
        df["_m"] = pd.to_numeric(
            df[mrp_col].astype(str).str.replace(",", "").str.strip(), errors="coerce"
        ).fillna(0)
    else:
        df["_m"] = 0.0
    ...
    if dist_col:
        df["_d"] = pd.to_numeric(
            df[dist_col].astype(str).str.replace(",", "").str.strip(), errors="coerce"
        ).fillna(0)
    else:
        df["_d"] = 0.0

    if "State Name" in df.columns and state_name:
        state_df = df[df["State Name"].astype(str).str.strip().str.lower() == state_name.strip().lower()]
...
# Replaces the previous run_azure_ocr / reconstruct_table / analyze_table /
# build_quantities pipeline.  The new pipeline:
#   1. run_document_intelligence_ocr(img_bytes) → raw detections list
#   2. match_detections_to_sku(raw_detections, prefix_index, sku_master)
#      → (quantities, match_log, line_items)
#
# Each raw detection has the shape:
#   { "description": str, "sku": str, "size": str, "quantity": str,
#     "formatted": str }
#
# The match_log entries consumed by build_live_log / the detection log tab
# retain the exact same keys as before so downstream code is unaffected:
#   { "product": str, "prefix": str, "size": str, "qty": int,
#     "full_sku": str, "status": "matched"|"unmatched" }
# ══════════════════════════════════════════════════════════════════════════════
...
def run_document_intelligence_ocr(img_bytes: bytes,
    ...
    Mirrors the logic from the standalone script provided by the user.
    Handles multi-table documents; skips header rows (row_index == 0 or 1).
    Only cells whose content is a positive integer are treated as quantities.
    """
    ...
    for table in result.tables:
        # ── Step 1: Build column-header map from row 0 and row 1 ────────────
        # OD size headers (15MM, 20MM …) are usually in row 1, col >= 2.
        # We attempt row 1 first; fall back to row 0 if row 1 yields nothing.
        ...
        # If we got nothing from rows 0-1, nothing useful to extract.
        if not header_map:
            continue

        # Normalise header values to canonical size tokens (e.g. "15" → "15MM")
        ...
        for col_idx, raw_hdr in header_map.items():
            ...
            canonical = SIZE_ALIASES.get(cleaned) or SIZE_ALIASES.get(
                ...
                raw_hdr,          # keep raw if no alias found
            )
            norm_header_map[col_idx] = canonical

        # ── Step 2: Build row-keyed lookup for description and SKU ──────────
        # description → col 0,  sku → col 1  (same convention as user script)
        ...
        # ── Step 3: Iterate data rows (row_index >= 2) ──────────────────────
        for cell in table.cells:
            ri = cell.row_index
            ci = cell.column_index
            ...
            if ri < 2:
                continue
            # Skip description/sku columns
            if ci < 2:
                continue
            # Only process columns that have a recognised size header
            if ci not in norm_header_map:
                continue

            content = cell.content.strip()
            # Accept only positive integer values as quantities
...
def match_detections_to_sku(
    ...
    Returns:
        ...
        match_log   – list of log-entry dicts (same schema as the old pipeline)
...
# ── build_live_log (unchanged logic, kept as-is) ──────────────────────────────
def build_live_log(
    match_log: list,
    quantities: dict,
    line_items: list,
...
for k, v in [
    ...
    ("raw_detections", []),           # ← new: stores raw DI detections
    ("bill_to", {}), ("ship_to", {}), ("party_confirmed", False), ("pdf_bytes", None),
    # Change 3: endpoint/key now resolved from Key Vault at runtime
    ...
    # Change 2: blob URLs replace local disk paths
    ("image_blob_url", ""),
    ("quotation_blob_url", ""),
    ("detection_blob_url", ""),
    ...
    # Step-3 state dropdown selections
    ("bill_state_dropdown", ""),
    ("ship_state_dropdown", ""),
]:
    _ss(k, v)
...
def render_step1():
    ...
    # ── Change 3: Azure OCR Settings shows Key Vault status, no manual entry ──
    with st.expander("🔧 Azure OCR / Key Vault Settings", expanded=False):
        ...
        if kv_endpoint and kv_key:
            ...
            # Allow manual override only when Key Vault is unavailable
...
def render_step2():
    ...
    # ── Change 2: Upload image to Azure Blob on first entry of Step 2 ─────────
    if (st.session_state.image_submitted
    ...
    if not done:
        ...
        if run:
            ...
            try:
                ...
                # ── Change 1: Use Document Intelligence prebuilt-layout ────────
...
buildTable();
        ...
        # ══════════════════════════════════════════════════════════════════════
        # CHANGE 1 — OCR DETECTION LOG TAB
        # Now shows data extracted by Azure Document Intelligence prebuilt-layout.
        # Colour-coding rules (identical semantics to the old tab):
        #   🟢 Green  — matched AND priced (MRP > 0)
        #   🟠 Orange — matched BUT MRP price is 0 / not in pricing file
        #   🔴 Red    — unmatched (SKU prefix could not be resolved)
        #
        # The raw detection column now also shows the Document Intelligence
        # "formatted" string (Description: SKU Prefix SIZE: QTY) so it is
        # easy to cross-check what the model actually read from the image.
        # ══════════════════════════════════════════════════════════════════════
        with tab_log:
            ...
            # Also expose the raw Document Intelligence detections for reference
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 3 — Dealer / Distributor Details (unchanged logic)
# ══════════════════════════════════════════════════════════════════════════════
def render_step3():
    ...
    # Auto-generate when all fields complete
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 4 — Preview & Download (unchanged logic, blob URL shown for reference)
# ══════════════════════════════════════════════════════════════════════════════
def render_step4():
