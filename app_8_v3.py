...
# ══════════════════════════════════════════════════════════════════════════════
# FIX — build_quantities
# Every OCR-extracted row emits one log entry per size detected.
# If a row has NO sizes at all, it still emits exactly ONE "unmatched" entry
# so the Detection Log always shows all recognised rows.
# ══════════════════════════════════════════════════════════════════════════════
def build_quantities(extracted_rows: list, pidx: dict, master: dict):
    ...
    for row in extracted_rows:
        ...
        if not sizes:
            # Row was seen by OCR but no quantity cell was resolved —
            # emit a single unmatched entry so the log stays complete.
            log.append({
                "product":  product,
                "prefix":   sku_prefix,
                "size":     "—",
        ...
        # Emit one log entry per (row × size) combination — NO deduplication.
        for sz, qty in sizes.items():
            full = match_sku(sku_prefix, sz, pidx, master)
            if full:
...
# FIX — build_live_log
# Rules:
#   1. Show EVERY entry in match_log without ANY deduplication.
#      Duplicate SKUs are fine — the log is a faithful mirror of what OCR found.
#   2. For matched entries, reflect the CURRENT (user-edited) quantity from the
#      quantities dict.
#   3. Append any manually-added items (SKUs that exist in line_items but do NOT
#      appear anywhere in match_log) at the end.
# ══════════════════════════════════════════════════════════════════════════════
def build_live_log(
    match_log: list,
    quantities: dict,
    ...
    # ── Step 1: Emit every match_log entry — no deduplication ─────────────────
    # Track which (resolved) SKUs appear in the OCR log so we can identify
    # manually-added ones later.
    skus_in_match_log: set = set()

    for entry in match_log:
        row = dict(entry)           # shallow copy — never mutate the original

        if row["status"] == "matched" and row["full_sku"] != "—":
            skus_in_match_log.add(row["full_sku"])
            # Reflect the current (possibly user-edited) quantity
            row["qty"] = quantities.get(row["full_sku"], row["qty"])

        live.append(row)

    # ── Step 2: Append manually-added items not found in match_log at all ─────
    # A manual item is one whose SKU does not appear ANYWHERE in match_log.
    seen_manual: set = set()
    for item in line_items:
        sku = item["sku"]
        if sku in skus_in_match_log:
            # Already represented by at least one match_log row above — skip.
            continue
...
buildTable();
        ...
        # Shows ALL entries from match_log without any deduplication.
        # Duplicates are expected and intentional — each OCR row is a separate entry.
        # Colour coding:
        #   🟢 Green  — matched AND has a non-zero MRP price
        #   🟠 Orange — matched BUT MRP price is 0 / not found in pricing file
        #   🔴 Red    — unmatched (SKU prefix could not be resolved to a catalogue SKU)
        # ══════════════════════════════════════════════════════════════════════
        with tab1:
            ...
            if live_log:
                ...
                st.markdown(
                    ...
                    f'📊 <b>Total OCR rows shown: {len(live_log)}</b> &nbsp;·&nbsp; '
                    f'Rows with detected quantity: <b>{total_rows_with_qty}</b> &nbsp;·&nbsp; '
                    f'Duplicates shown as-is (OCR-faithful)'
                    f'</div>',
                    unsafe_allow_html=True,
                )

                # ── Build HTML table — one row per match_log entry ────────────
                h = (
                ...
                for idx, entry in enumerate(live_log, 1):
                    ...
                    # Resolved SKU cell: code tag when valid, dash when not
                    sku_display = (
                    ...
                    # Qty display: bold green if > 0, muted dash if 0
                    qty_display = (
...
def render_step3():
    ...
    # ── Build state options list — always starts with blank option ────────────
    state_options = ALL_INDIA_STATES  # already starts with ""
...
def _reset_session():
    ...
    # Reset state dropdown selections
