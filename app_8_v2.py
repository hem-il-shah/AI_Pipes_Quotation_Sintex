...
def build_quantities(extracted_rows: list, pidx: dict, master: dict):
    ...
    for row in extracted_rows:
        if not row["sizes"]:
            # Row was detected by OCR but no quantity cells resolved —
            # still emit it to the log so the Detection Log shows it.
            log.append({
                "product":  row["product"],
...
def build_live_log(
    ...
    """
    Return a list of EVERY detected entry to display in the OCR Detection Log.

    - Every entry in match_log is emitted exactly once (original OCR row order).
    - Unmatched entries are kept as-is with status "unmatched".
    - Matched entries show the CURRENT qty from quantities dict (reflects edits).
    - No deduplication: if the same SKU appears multiple times in match_log,
      all occurrences are shown as separate rows.
    - Manually-added SKUs not in match_log are appended at the end.
    """
    live: list = []

    # ── Collect all SKUs that appear in match_log (matched only) ─────────────
    # We need this set only to identify manual additions at the end.
    ...
    # ── Emit every match_log entry without any deduplication ─────────────────
    for entry in match_log:
        row = dict(entry)  # shallow copy so we don't mutate original
        ...
        # Always append — unmatched, matched with price, matched without price
        live.append(row)

    # ── Append manually-added items not originating from OCR ─────────────────
    # A manually-added item is one whose SKU does NOT appear anywhere in
    # match_log at all.
    seen_manual: set = set()
    for item in line_items:
        sku = item["sku"]
        if sku in skus_in_match_log:
            # Already represented via match_log rows above
            continue
...
def render_step3():
    ...
    # Use ALL_INDIA_STATES as fallback; always include empty string as first option
    ...
    # ── Bill To State — dropdown (FIX for Update-2) ───────────────────────────
    ...
    # ── Ship To State — dropdown (FIX for Update-2) ───────────────────────────
    ...
    # ── Show which mandatory fields are still missing ─────────────────────────
    missing_hints = []
