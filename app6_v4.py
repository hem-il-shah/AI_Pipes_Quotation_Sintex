...
/* State selector card */
.state-selector-card{background:white;border-radius:var(--radius);box-shadow:var(--shadow);
  margin-bottom:20px;overflow:hidden;border:1px solid var(--border);}
.state-selector-header{background:linear-gradient(90deg,var(--red),#D94644);padding:14px 18px;
  display:flex;align-items:center;gap:12px;}
.state-selector-body{padding:18px;}
...
for k, v in [
    ...
    ("ocr_reviewed", False),
    ("upload_key", 0),
    ("selected_state", "Chhattisgarh"),   # CHANGE 1: state selection
    ("state_confirmed", False),           # CHANGE 1: gate for image capture
]:
    _ss(k, v)
...
# CHANGE 1: Load MRP data for the currently selected state
mrp_data = load_mrp_data_for_state(st.session_state.selected_state)

# CHANGE 1: Load state names from CSV (with ALL_INDIA_STATES as fallback)
...
# ═══════════════════════════════════════════════════════════════════════════════
# CHANGE 1: STATE SELECTOR — shown above Step 1, before image capture
# ═══════════════════════════════════════════════════════════════════════════════

def render_state_selector():
    """
    Renders a prominent state-selector card above the image capture step.
    Once a state is chosen, MRP data is reloaded for that state automatically.
    The selector remains visible (not locked) so the user can change state at any time
    before submitting the image.
    """
    # Find the index of the currently selected state in available_states
    try:
    ...
    <div class="state-selector-card">
      <div class="state-selector-header">
        ...
        <div>
          <div class="step-title">Select State / Region</div>
          <div class="step-subtitle">MRP &amp; distributor pricing will be loaded for the chosen state</div>
        </div>
      </div>
    </div>""", unsafe_allow_html=True)

    with st.container():
        st.markdown('<div style="background:white;border-radius:0 0 12px 12px;'
                    'border:1px solid #DEDEDE;border-top:none;padding:16px 18px;'
                    'margin-top:-20px;margin-bottom:20px;">', unsafe_allow_html=True)
        ...
        if chosen != st.session_state.selected_state:
            ...
            # Reload MRP data for new state — clear cache entry so fresh data loads
            load_mrp_data_for_state.clear()
            # Also reset downstream state if image not yet submitted
        ...
        st.markdown(
            f'<div class="info-box" style="margin-top:8px;">📍 Selected: '
...
def render_step1():
    ...
    if st.session_state.image_bytes is not None:
        ...
        b64 = _b64.b64encode(st.session_state.image_bytes).decode()

        with st.expander("🖼️ Preview Uploaded Image", expanded=True):
            st.markdown(
                f'<div class="img-preview-container">'
                f'<img src="data:image/jpeg;base64,{b64}" alt="Order form preview"/>'
                f'</div>', unsafe_allow_html=True)

        if not st.session_state.image_submitted:
            st.markdown('<div class="success-box">✅ Image loaded — click <b>Submit</b> to proceed to OCR analysis</div>',
                        unsafe_allow_html=True)
            col_btn, col_retake = st.columns([3, 1])
            with col_btn:
                # if st.button("✅  Submit & Proceed to OCR →", key="submit_img"):
                if st.button("✅  Display Quotation →", key="submit_img"):
...
def render_step2():
    # CHANGE 2: Return early (render nothing) if step is locked
...
def render_step3():
    ...
    # CHANGE 3: Party form WITHOUT the customer lookup selectbox
...
# CHANGE 1: State selector rendered first, before Step 1
