...
def ensure_tables_exist():
    """
    Create dbo.ocr_logs and dbo.quotation_logs if they don't already exist.
...
def db_insert_ocr_log(
    ...
    """
    Insert a new OCR-started row into dbo.ocr_logs.
    Returns the new ocr_id (IDENTITY value), or None on failure.
...
def db_update_ocr_log(
    ...
    """Update the ocr_logs row after OCR completes."""
    sql = """
...
def db_insert_quotation_log(
    ...
    """Insert one row into dbo.quotation_logs."""
    sql = """
...
# ── Legacy log xlsx (kept for backward compat, DB is primary) ─────────────────
...
/* Hide the geo-bridge text input completely */
[data-testid="stTextInput"][data-geo-bridge="true"] {
...
# ── Initialise DB tables on first load ────────────────────────────────────────
ensure_tables_exist()
...
#
# Strategy:
#   1. An invisible Streamlit text_input ("_geo_bridge") is rendered.
#   2. A components.html snippet fires navigator.geolocation.getCurrentPosition
#      and locates that exact hidden <input> element in the parent Streamlit
#      document by its aria-label, then sets its value + dispatches a React
#      synthetic 'input' event so Streamlit registers the change.
#   3. On the next Streamlit rerun (triggered by the input change), Python reads
#      the JSON payload from st.session_state["_geo_bridge"] and stores
#      lat/lng into session state.
#   4. The component is only injected once — when geo_status == "pending".
# ══════════════════════════════════════════════════════════════════════════════
...
def _parse_geo_bridge():
    ...
    Called once per rerun, before rendering any step.
    """
...
def render_geolocation_component():
    ...
    Renders:
      ...
      - A components.html block (height=0) containing the JS that:
          a) calls navigator.geolocation.getCurrentPosition
          b) writes JSON into the hidden input and fires a React 'input' + 'change'
             event so Streamlit registers the value change and triggers a rerun.

    The component is injected only while geo_status == "pending" to avoid
    repeated permission prompts.
    """
    # ── Render the invisible bridge input (always present so its key exists) ──
    # We hide it with CSS; Streamlit still reads its value via session_state.
    st.text_input(
    ...
    # ── Only inject JS while we are still waiting for a result ───────────────
...
(function () {
    ...
    // ── Helper: write value into the hidden Streamlit text input ─────────────
    // Streamlit uses React internally, so we must trigger both the native
    // input event AND a React synthetic event (via the nativeInputValueSetter).
    function writeToStreamlitInput(selector, value) {
        try {
            var doc = window.parent.document;
            var el  = doc.querySelector(selector);
            if (!el) {
                // Fallback: search all inputs by their aria-label
            ...
            // Use React's internal setter to bypass the read-only value property
            var nativeInputValueSetter = Object.getOwnPropertyDescriptor(
                window.HTMLInputElement.prototype, 'value'
            ).set;
            nativeInputValueSetter.call(el, value);

            // Fire both events so React / Streamlit register the change
    ...
    function sendResult(lat, lng, status) {
        ...
        // Primary selector: Streamlit text inputs have data-testid="stTextInput"
        var written = writeToStreamlitInput(
            'input[aria-label="__geo_bridge_hidden__"]',
            payload
        );
        if (!written) {
            // The label may be rendered differently; try a broader search
            setTimeout(function () {
                writeToStreamlitInput(
                    'input[aria-label="__geo_bridge_hidden__"]',
                    payload
                );
            }, 800);
...
def _geo_status_badge() -> str:
    ...
    if status == "ok" and lat is not None and lng is not None:
        return (
            ...
            f'{lat:.5f}, {lng:.5f}</span></div>'
        )
    elif status == "denied":
        return (
            '<div class="geo-badge denied">'
            '🚫 Location permission denied — coordinates will not be recorded</div>'
        )
    elif status in ("unavailable", "timeout"):
        return (
            '<div class="geo-badge denied">'
            '⚠️ Location unavailable — coordinates will not be recorded</div>'
        )
    else:
        return (
            '<div class="geo-badge pending">'
            '⏳ Awaiting location permission…</div>'
        )


# ── File helpers ───────────────────────────────────────────────────────────────
def save_image_to_disk(raw_bytes: bytes, rotation: int = 0) -> str:
    """Save captured/uploaded image to images/ folder. Returns absolute path."""
...
def save_quotation_pdf_to_disk(pdf_bytes: bytes, quotation_id: str) -> str:
    """Save final quotation PDF to quotations/ folder. Returns absolute path."""
...
def save_detection_pdf_to_disk(pdf_bytes: bytes, quotation_id: str) -> str:
    """Save a copy of the PDF (representing the OCR detection result)
    to detections/ folder. Returns absolute path."""
...
def append_log_entry(quotation_id: str, pdf_path: str, bill_to: dict, ship_to: dict,
    ...
    """Append to the legacy Excel log (backup / offline record)."""
    _ensure_log_xlsx()
...
for k, v in [
    ...
    ("quotation_disk_path", ""),
    ("detection_disk_path", ""),
    # ── Geolocation ── (geo bridge key initialised separately below)
    ("user_latitude", None),
    ("user_longitude", None),
    ("geo_status", "pending"),
]:
...
# Geo bridge text_input key must be initialised before the widget renders
...
# ══════════════════════════════════════════════════════════════════════════════
# GEOLOCATION — Parse bridge value FIRST, then render the component
# This ordering is critical: parse on every rerun so that when Streamlit
# triggers a rerun due to the text_input change, we pick up the result.
# ══════════════════════════════════════════════════════════════════════════════
_parse_geo_bridge()
...
def render_step1():
    ...
    st.markdown(_geo_status_badge(), unsafe_allow_html=True)
...
(function(){
    ...
    if raw_bytes_pending is not None:
        ...
        if st.button("✅  Submit & Process Image", key="btn_submit_image"):
            ...
            # Reset DB tracking flags for the new run
...
def render_step2():
    ...
    # ── Save image to disk + insert OCR-started row in DB ────────────────────
    if (st.session_state.image_submitted
        ...
        try:
            ...
            # Insert pending OCR row immediately so we have an ocr_id
    ...
    if not done:
        ...
        if run:
            ...
            try:
                ...
                # ── Update ocr_logs row with results ─────────────────────────
                ...
                # Mark OCR as failed in DB — correct 5-arg signature
                if st.session_state.db_ocr_id:
                    db_update_ocr_log(
                        ocr_id=st.session_state.db_ocr_id,
                        quantities_detected=0,
                        matched_count=0,
                        unmatched_count=0,
                        status=2,
                    )
        ...
        if skip:
            ...
            st.session_state.ocr_done   = True
            # Update DB row as manually-skipped — correct 5-arg signature
            if st.session_state.db_ocr_id:
                db_update_ocr_log(
                    ocr_id=st.session_state.db_ocr_id,
                    quantities_detected=0,
...
def render_step3():
    ...
    def do_confirm(b: dict, s: dict):
        ...
        2. Build PDF bytes
        3. Save quotation PDF  → quotations/
        4. Save detection copy → detections/
        5. Write quotation_logs row to SQL (includes lat/lng from session_state)
        6. Write legacy Excel log (backup)
        ...
        with st.spinner("Generating PDF…"):
            try:
                ...
                # ── 5. Write to SQL quotation_logs (with lat/lng) ──────────
...
def render_step4():
    ...
    if quotation_id:
        # ── Show location info captured at time of quotation ──────────────────
        lat    = st.session_state.get("user_latitude")
        lng    = st.session_state.get("user_longitude")
    ...
    st.markdown('<div class="success-box">✅ Quotation ready — saved to <code>quotations/</code> &amp; <code>detections/</code></div>',
                unsafe_allow_html=True)
...
def _reset_session():
    ...
    st.session_state["quotation_disk_path"]      = ""
    st.session_state["detection_disk_path"]      = ""
    # NOTE: Geolocation is intentionally NOT reset — the user's physical
    # location doesn't change between quotations in the same session.
    # geo_status, user_latitude, user_longitude are preserved.
    st.rerun()
