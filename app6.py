...
.progress-bar-wrap{display:flex;justify-content:space-between;align-items:center;
  margin-bottom:24px;gap:4px;}
.progress-step{flex:1;display:flex;flex-direction:column;align-items:center;gap:4px;}
.progress-step-dot{width:28px;height:28px;border-radius:50%;display:flex;align-items:center;
  justify-content:center;font-size:12px;font-weight:700;border:2px solid var(--mgray);
  background:white;color:var(--muted);}
.progress-step-dot.active{background:var(--red);border-color:var(--red);color:white;}
.progress-step-dot.done{background:var(--green);border-color:var(--green);color:white;}
.progress-step-line{flex:1;height:2px;background:var(--mgray);margin-top:-20px;}
.progress-step-line.done{background:var(--green);}
.progress-step-label{font-size:10px;color:var(--muted);text-align:center;font-weight:500;}
.progress-step-label.active{color:var(--red);font-weight:700;}
.progress-step-label.done{color:var(--green);font-weight:600;}
...
def build_pdf(quantities: dict, mrp_data: dict, bill_to: dict,
              ship_to: dict, sku_master: dict) -> bytes:
    buf = io.BytesIO()
    doc = SimpleDocTemplate(buf, pagesize=landscape(A4),
                            leftMargin=10*mm, rightMargin=10*mm,
                            topMargin=10*mm, bottomMargin=10*mm)
    RED   = colors.HexColor("#C0211F"); DRED  = colors.HexColor("#8B1514")
    LGRAY = colors.HexColor("#F5F5F5"); MGRAY = colors.HexColor("#DEDEDE")
    WHITE = colors.white;               BLACK = colors.HexColor("#1A1A1A")
    GREEN = colors.HexColor("#1E7E4A")
    sty   = getSampleStyleSheet()
    def ps(n, **kw): return ParagraphStyle(n, parent=sty["Normal"], **kw)
    sm   = ps("sm",  fontName="Helvetica",     fontSize=7,   textColor=colors.HexColor("#555"))
    ...
    sch  = ps("sch", fontName="Helvetica-Bold",fontSize=7,   textColor=WHITE)

    story = []

    hdr = Table([[
        Paragraph("<b>Sintex BAPL Limited</b><br/>Kutesar Road, Raipur, CG 492101<br/>GSTIN: 22AADCB1921F1ZE",
                  ps("h1",fontName="Helvetica-Bold",fontSize=8,textColor=WHITE)),
        Paragraph("<b>SALES QUOTATION</b>",
                  ps("h2",fontName="Helvetica-Bold",fontSize=16,textColor=WHITE,alignment=TA_CENTER)),
        Paragraph("State: <b>Chhattisgarh</b>",
                  ps("h3",fontName="Helvetica",fontSize=8,textColor=WHITE,alignment=TA_RIGHT)),
    ]], colWidths=[80*mm,100*mm,77*mm])
    hdr.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1),RED),("TEXTCOLOR",(0,0),(-1,-1),WHITE),
    ...
    pt = Table([
        [Paragraph("<b>BILL TO PARTY</b>",ps("b1",fontName="Helvetica-Bold",fontSize=8,textColor=WHITE)),
         Paragraph("<b>SHIP TO PARTY</b>",ps("s1",fontName="Helvetica-Bold",fontSize=8,textColor=WHITE))],
        [pp(bill_to), pp(ship_to)],
    ], colWidths=[128.5*mm,128.5*mm])
    pt.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),DRED),("TEXTCOLOR",(0,0),(-1,0),WHITE),
                             ("BACKGROUND",(0,1),(-1,1),LGRAY),("BOX",(0,0),(-1,-1),0.5,MGRAY),
                             ("GRID",(0,0),(-1,-1),0.5,MGRAY),("VALIGN",(0,0),(-1,-1),"TOP"),
                             ("PADDING",(0,0),(-1,-1),6)]))
    story += [pt, Spacer(1,5*mm)]

    heads = ["S.No","Sheet","Product","SKU Code","OD","Inch","MRP(₹)","Qty","Total MRP","Dist.Landing","Taxable"]
    trows = [[Paragraph(h,sch) for h in heads]]
    ln = gm = gd = gt = 0
    for sku, qty in {s:q for s,q in quantities.items() if q>0}.items():
        info  = sku_master.get(sku,{});  mi = mrp_data.get(sku,{})
        mrp   = mi.get("mrp",0.);       dist = mi.get("distributor_landing",0.)
        tot   = round(mrp*qty,2);       tax  = round(dist*qty,2)
        ln+=1; gm+=tot; gd+=(mrp-dist)*qty; gt+=tax
        trows.append([
            Paragraph(str(ln),cell),
            Paragraph(info.get("sheet",""),cell),
            Paragraph(info.get("product",""),cell),
            Paragraph(sku,ps("sk",fontName="Courier",fontSize=7,textColor=BLACK)),
            Paragraph(info.get("od_size",""),cell),
            Paragraph(info.get("inch_size",""),cell),
            Paragraph(f"{mrp:,.2f}",celr),
            Paragraph(str(qty),ps("qc",fontName="Helvetica-Bold",fontSize=8,textColor=BLACK,alignment=TA_CENTER)),
            Paragraph(f"{tot:,.2f}",celb),
            Paragraph(f"{dist:,.2f}",celr),
            Paragraph(f"{tax:,.2f}",celb),
        ])
    if ln == 0:
        trows.append([Paragraph("No items",cell)]+[""]*10)

    cw = [9*mm,28*mm,47*mm,26*mm,13*mm,11*mm,17*mm,9*mm,19*mm,20*mm,20*mm]
    t2 = Table(trows, colWidths=cw, repeatRows=1)
    ts = [("BACKGROUND",(0,0),(-1,0),RED),("TEXTCOLOR",(0,0),(-1,0),WHITE),
          ("GRID",(0,1),(-1,-1),0.4,MGRAY),("BOX",(0,0),(-1,-1),0.8,RED),
          ("VALIGN",(0,0),(-1,-1),"MIDDLE"),
          ("TOPPADDING",(0,0),(-1,-1),4),("BOTTOMPADDING",(0,0),(-1,-1),4),
          ("LEFTPADDING",(0,0),(-1,-1),4),("RIGHTPADDING",(0,0),(-1,-1),4)]
    for ri in range(1,len(trows)):
        if ri%2==0: ts.append(("BACKGROUND",(0,ri),(-1,ri),LGRAY))
    t2.setStyle(TableStyle(ts)); story += [t2, Spacer(1,4*mm)]

    tdata = [
        ["","","","","","","",
         Paragraph("Gross Total (MRP):",ps("tl",fontName="Helvetica-Bold",fontSize=8)),
         Paragraph(f"₹ {gm:,.2f}",ps("tv",fontName="Helvetica-Bold",fontSize=8,alignment=TA_RIGHT)),"",""],
        ["","","","","","","",
         Paragraph("Less Distributor Discount:",ps("tl2",fontName="Helvetica",fontSize=8,textColor=DRED)),
         Paragraph(f"- ₹ {gd:,.2f}",ps("tv2",fontName="Helvetica-Bold",fontSize=8,textColor=DRED,alignment=TA_RIGHT)),"",""],
        ["","","","","","","",
         Paragraph("Net Taxable Value:",ps("tl3",fontName="Helvetica-Bold",fontSize=9,textColor=GREEN)),
         Paragraph(f"₹ {gt:,.2f}",ps("tv3",fontName="Helvetica-Bold",fontSize=9,textColor=GREEN,alignment=TA_RIGHT)),"",""],
    ]
    tot = Table(tdata, colWidths=cw)
    tot.setStyle(TableStyle([("BOX",(7,0),(8,-1),0.5,MGRAY),("GRID",(7,0),(8,-1),0.5,MGRAY),
                              ("BACKGROUND",(7,2),(8,2),colors.HexColor("#ECFDF5")),
                              ("VALIGN",(0,0),(-1,-1),"MIDDLE"),
                              ("TOPPADDING",(0,0),(-1,-1),3),("BOTTOMPADDING",(0,0),(-1,-1),3)]))
    story += [tot, Spacer(1,3*mm)]

    ft = Table([[
        Paragraph("<i>Computer-generated quotation. Subject to change without notice.</i>",
                  ps("ft",fontName="Helvetica-Oblique",fontSize=7,textColor=colors.HexColor("#888"))),
        Paragraph("<b>Authorised Signatory</b>",
                  ps("sg",fontName="Helvetica-Bold",fontSize=8,alignment=TA_RIGHT)),
    ]], colWidths=[180*mm,77*mm])
    ft.setStyle(TableStyle([("LINEABOVE",(0,0),(-1,0),0.5,MGRAY),
                             ("VALIGN",(0,0),(-1,0),"MIDDLE"),("TOPPADDING",(0,0),(-1,0),5)]))
    story.append(ft)
    doc.build(story)
...
def progress_bar():
    steps = ["📷 Capture","⚡ OCR Review","👤 Parties","📄 Download"]
    cur   = st.session_state.step
    html  = ""
    for i, lbl in enumerate(steps, 1):
        dc = "done" if i<cur else ("active" if i==cur else "")
        di = "✓"   if i<cur else str(i)
        lc = "done" if i<cur else ("active" if i==cur else "")
        html += (f'<div class="progress-step">'
                 f'<div class="progress-step-dot {dc}">{di}</div>'
                 f'<div class="progress-step-label {lc}">{lbl}</div></div>')
        if i < len(steps):
            html += f'<div class="progress-step-line {"done" if i<cur else ""}" style="margin-bottom:18px"></div>'
    st.markdown(f'<div class="progress-bar-wrap">{html}</div>', unsafe_allow_html=True)
...
def render_step2():
    ...
    if not done:
        ...
        if run:
            ...
            try:
                ...
                qtys, log = build_quantities(res["rows"], prefix_index, sku_master)
                ...
                if nm == 0:
                    ...
                    ph.success(f"✅ Done — {nm} pairs matched, {nu} unmatched. Review below.")
    ...
    if done:
        ...
        nm = sum(1 for m in log if m["status"]=="matched")
        nu = sum(1 for m in log if m["status"]=="unmatched")
        ni = sum(1 for q in qtys.values() if q>0)

        if nm: st.markdown(f'<div class="success-box">✅ {nm} SKU-size pairs matched → {ni} line items</div>',
                           unsafe_allow_html=True)
        ...
        with tab1:
            if any(q>0 for q in qtys.values()):
                ...
                <thead><tr><th class="L">Product</th><th class="L">SKU</th>
                <th>OD</th><th>Inch</th><th>Sheet</th><th>Qty</th>
                <th>MRP(₹)</th><th>Dist.Landing(₹)</th></tr></thead><tbody>"""
                for sku, qty in sorted(qtys.items()):
                    if qty<=0: continue
                    info=sku_master.get(sku,{}); mi=mrp_data.get(sku,{})
                    h += (f'<tr><td class="L">{info.get("product","?")}</td>'
                          f'<td class="M">{sku}</td><td>{info.get("od_size","")}</td>'
                          f'<td>{info.get("inch_size","")}</td>'
                          f'<td style="font-size:10px">{info.get("sheet","")}</td>'
        ...
        <div class="totals-box">
          <div class="total-row"><span class="total-lbl">Line items</span><span class="total-val">{len(iord)}</span></div>
...
def render_step4():
    ...
    <div class="step-card"><div class="step-card-header">
      ...
      <div><div class="step-title">Step 4 — Download Quotation</div>
      <div class="step-subtitle">Your Sales Quotation PDF is ready</div></div>
    </div><div class="step-body">""", unsafe_allow_html=True)

    items = {s:q for s,q in st.session_state.quantities.items() if q>0}
    gmrp  = sum(mrp_data.get(s,{}).get("mrp",0)*q  for s,q in items.items())
    gdist = sum(mrp_data.get(s,{}).get("distributor_landing",0)*q for s,q in items.items())
    disc  = gmrp - gdist

    st.markdown(f"""
    <div class="success-box">✅ Quotation ready — <b>{len(items)} line items</b></div>
    <div class="totals-box">
      <div class="total-row"><span class="total-lbl">Total SKUs</span><span class="total-val">{len(items)}</span></div>
      <div class="total-row"><span class="total-lbl">Gross MRP</span><span class="total-val">₹ {gmrp:,.2f}</span></div>
      <div class="total-row"><span class="total-lbl">Distributor Discount</span><span class="total-val neg">− ₹ {disc:,.2f}</span></div>
      <div class="total-row grand"><span class="total-lbl">Net Distributor Landing</span><span class="total-val">₹ {gdist:,.2f}</span></div>
    </div>""", unsafe_allow_html=True)

    bill = st.session_state.bill_to; ship = st.session_state.ship_to
    st.markdown(f"""
    <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:14px;">
      <div style="flex:1;min-width:200px;background:#FFF0F0;border:1.5px solid #F5C0C0;border-radius:10px;padding:12px;">
        <div style="font-size:11px;font-weight:700;color:#C0211F;text-transform:uppercase;margin-bottom:6px;">🏢 Bill To</div>
        <div style="font-size:12.5px;font-weight:600;">{bill.get('name','').split(chr(10))[0]}</div>
        <div style="font-size:11px;color:#555;">{bill.get('gst','')}</div>
      </div>
      <div style="flex:1;min-width:200px;background:#F0FFF0;border:1.5px solid #C0E8C0;border-radius:10px;padding:12px;">
        <div style="font-size:11px;font-weight:700;color:#1E7E4A;text-transform:uppercase;margin-bottom:6px;">🚚 Ship To</div>
        <div style="font-size:12.5px;font-weight:600;">{ship.get('name','').split(chr(10))[0]}</div>
        <div style="font-size:11px;color:#555;">{ship.get('gst','')}</div>
      </div>
    </div>""", unsafe_allow_html=True)
    ...
    if st.session_state.pdf_bytes:
        ...
        st.download_button("📥  Download PDF (Alternative)", data=st.session_state.pdf_bytes,
                           file_name="sintex_quotation.pdf", mime="application/pdf")
    ...
    with cn:
        if st.button("🔄  New Quotation", key="newq"):
            ...
            for k in ["match_log","ocr_grid","ocr_extracted"]: st.session_state[k] = []
